...
if (isset($request["VK"]))
{
	...
	if (!isset($request["VK"]["VK_SETTINGS"]["GROUP_ID"]) || strlen($request["VK"]["VK_SETTINGS"]["GROUP_ID"]) <= 0)
...
//	todo: somtehing wrong...
	if ($exportId && $request["VK"]["EXPORT_SETTINGS"]["CATEGORY_DEFAULT"] <= 0 && $vk->isActiveById($exportId))
	{
...
//	SELECTOR for vk-category change
	if ($vkSettings["EXPORT_SETTINGS"]["CATEGORY_DEFAULT"] > 0)
		$vkCategorySelected = $vkSettings["EXPORT_SETTINGS"]["CATEGORY_DEFAULT"];


//	prepare CATEGORY selector
//	in this method we use API request - must be checked execution errors
	try
	{
...
//			logger may generate exception - catch them and do nothing
			try
			{
				$logger = new Vk\Logger($exportId);
				$logger->addError('WRONG_ACCESS_TOKEN');
				unset($logger);
			}
			catch (Vk\ExecuteException $e)
			{
...
<? endif; ?>
	...
	<form name="vk_exhangesettings_form" method="post" action="<?= $currPageUrl ?>">
		...
		<tr class="adm-detail-required-field">
			...
			<td>
				<input type="text" name="VK[VK_SETTINGS][GROUP_ID]" size="25" maxlength="255"
					   value="<?= isset($vkSettings["VK_SETTINGS"]["GROUP_ID"]) ? $vkSettings["VK_SETTINGS"]["GROUP_ID"] : "" ?>">
			</td>
		</tr>
		...
		<!--		get TOKEN-->
		<? if (
			isset($vkSettings["VK_SETTINGS"]["GROUP_ID"]) && !empty($vkSettings["VK_SETTINGS"]["GROUP_ID"]) &&

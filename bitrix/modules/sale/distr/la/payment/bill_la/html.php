...
<div style="text-align: center; font-size: 2em"><b>Factura</b></div>
...
if ($arBasket = $dbBasket->Fetch())
{
	...
	do
	{
		...
		$arCells[++$n] = array(
			...
			$arBasket["MEASURE_NAME"] ? htmlspecialcharsbx($arBasket["MEASURE_NAME"]) : 'pza',
...
<table class="it" width="100%">
	<tr>
		<td><nobr>#</nobr></td>
		<td><nobr>Artнculo / Descripciуn</nobr></td>
	...
	<tr>
		...
		<? if ($bankAccNo && $bankRouteNo && $bankSwift) { ?>
			...
			<? if (CSalePaySystemAction::GetParamValue("SELLER_NAME", false)) { ?>
				Nombre de la cuenta: <?=CSalePaySystemAction::GetParamValue("SELLER_NAME", false); ?>
				<br>
			<? } ?>

			# de la Cuenta: <?=$bankAccNo; ?>
			<br>
			...
			<? if ($bank || $bankAddr || $bankPhone) { ?>
				Nombre y direcciуn del banco: <? if ($bank) { ?><?=$bank; ?><? } ?>
				<br>
			...
			Nъmero de enrutamiento del banco: <?=$bankRouteNo; ?>
			<br>

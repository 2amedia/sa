...
{
	$pdf->Write(15, CSalePdf::prepareToPdf('Para'));
}
...
$title = CSalePdf::prepareToPdf('Girada el: ');
$titleWidth = $pdf->GetStringWidth($title);
$pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
$pdf->Write(15, $title, 0, 0, 'R');
$pdf->Ln();
...
if (CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false))
{
	...
	$title = CSalePdf::prepareToPdf('Para pagarse el: ');
	$titleWidth = $pdf->GetStringWidth($title);
	$pdf->SetX($pdf->GetX() - $invoiceInfoWidth - $titleWidth - 6);
	$pdf->Write(15, $title, 0, 0, 'R');
}
...
if ($arBasket = $dbBasket->Fetch())
{
	...
	else
	{
		...
		while ($arTaxList = $dbTaxList->Fetch())
		{
			$arCells[++$n] = array(
				...
				CSalePdf::prepareToPdf(sprintf(
					"%s%s%s:",
					($arTaxList["IS_IN_PRICE"] == "Y") ? "Incluido " : "",
					$arTaxList["TAX_NAME"],
	...
	if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SUM_PAID"]) > 0)
	{
		$arCells[++$n] = array(
			...
			null,
			null,
			CSalePdf::prepareToPdf("Pago hecho:"),
			CSalePdf::prepareToPdf(SaleFormatCurrency(
...
{
	$pdf->Write(15, CSalePdf::prepareToPdf('Tйrminos y Condiciones'));
	$pdf->Ln();
...
if ($bankAccNo && $bankRouteNo && $bankSwift)
{
	...
	$pdf->Write(15, CSalePdf::prepareToPdf("Detalles bancarios"));
	$pdf->Ln();
	...
	$bankDetails .= CSalePdf::prepareToPdf(sprintf("# de la Cuenta: %s\n", $bankAccNo));
	...
	{
		$bankDetails .= CSalePdf::prepareToPdf("Nombre y direcciуn del banco: ");
		if ($bank)
	...
	$bankDetails .= CSalePdf::prepareToPdf(sprintf("Nъmero de enrutamiento del banco: %s\n", $bankRouteNo));

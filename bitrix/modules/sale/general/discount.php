		...
		foreach (array_keys($arOrder['BASKET_ITEMS']) as $index)
		{
			...
			if (!CSaleBasketHelper::isSetItem($arShoppingCartItem))
			{
				...
				if ($arShoppingCartItem["BASE_PRICE"] > 0)
					$arShoppingCartItem["DISCOUNT_PRICE_PERCENT"] = ($arShoppingCartItem["DISCOUNT_PRICE"] * 100) / $arShoppingCartItem["BASE_PRICE"] + $arShoppingCartItem["PRICE"];
				...
				if (!empty($discountResult['BASKET_ROUND'][$code]))
					$simplePercent = false;

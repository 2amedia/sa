...
foreach ($params['BASKET_ITEMS'] as $basketItem)
{
	...
	if ($basketItem['IS_VAT_IN_PRICE'])
	{
		...
		{
			$basketItemSum = ($basketItem['VAT_RATE'] != 0.0) ?
				$basketItemTotal / (1 + doubleval($basketItem['VAT_RATE'])) : $basketItemTotal;
...
if ($params['DELIVERY_PRICE'] > 0)
{
	...
	$basketItemSum = doubleval($params['DELIVERY_PRICE']);
	if (($vat != 0.0))
	{
		$basketItemTotal = doubleval($basketItemSum * (1 + doubleval($vat)));
		$basketItemVatSum = doubleval($basketItemSum * doubleval($vat));
	}
	else
	{
		$basketItemTotal = $basketItemSum;

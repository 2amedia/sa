...
class ImportOneCPackage extends ImportPattern
{
    ...
    protected function modifyEntity($item)
    {
        ...
        if(!($item instanceof Exchange\Entity\EntityImport) && !($item instanceof ProfileImport))
            throw new ArgumentException("Item must be instanceof EntityImport or ProfileImport");
    ...
    protected function convert(array $documents)
    {
		...
		foreach($documents as $document)
		{
			if($document->getOwnerEntityTypeId() == EntityType::ORDER)
			{
				...
				$documentProfile = new OneC\ProfileDocument(EntityType::PROFILE);
    ...
    public static function configuration()
    {
        ...
        Manager::registerInstance(EntityType::PROFILE, OneC\ImportSettings::getCurrent(), new OneC\CollisionProfile(), new OneC\CriterionProfile());
    }
    ...
    protected function sortItems(array $items)
    {
        ...
        foreach ($items as $item)
        {
            if($item->getOwnerTypeId() == EntityType::PROFILE)
            {
                $list[$i++] = $item;
            }
        }
        ...
        foreach ($items as $item)
        {
            if($item->getOwnerTypeId() <> EntityType::ORDER && $item->getOwnerTypeId() <> EntityType::PROFILE)
            {
                $list[$i++] = $item;
            }
        }
    ...
    protected function import(array $items)
    {
		...
		if(!$this->hasCollisionErrors($items))
		{
			...
			foreach($items as $item)
			{
				if($item->getOwnerTypeId() == EntityType::PROFILE)
				{
					/** @var Exchange\ProfileImport $item */
					$params = $item->getFieldValues();
					$fields = $params['TRAITS'];
					...
					if($r->isSuccess())
					{
						/** @var Exchange\ProfileImport $profile */
						$profile = $item->getEntity();
						$personalTypeId = $profile->getPersonalTypeId();
						...
						if($r->isSuccess())
						{
							...
							if(!empty($property))
							{
								$propertyByConfig = $item->getPropertyOrdersByConfig($personalTypeId, $profile->getField('USER_PROPS'), $property);
								$fieldsOrder['ORDER_PROP'] = $propertyByConfig;
								$params['ORDER_PROP'] = $propertyByConfig;
							}

							$fieldsOrder['USER_ID'] = $userId;
    ...
    protected function onBeforeShipmentCollectionModify(Exchange\Entity\OrderImport $orderImport, $items)
    {
        ...
        if(!empty($shipmentList))
        {
            foreach($shipmentList as $id=>$shipment)
            {
				...
				/** @var ICollisionShipment $collision */
				$collision->resolve($item);
    ...
    protected function onBeforePaymentCollectionModify(Exchange\Entity\OrderImport $orderImport, $items)
    {
        ...
        if(!empty($paymentList))
        {
            foreach($paymentList as $id=>$payment)
            {
                ...
                /** @var ICollisionPayment $collision */
				$collision->resolve($item);
				if(!$item->hasCollisionErrors())
                {
                    $result = $item->delete();
	...
	private function UpdateCashBoxChecks(Exchange\Entity\OrderImport $orderImport, array $items)
	{
		...
		$collision->resolve($orderImport);

		if($orderImport->hasCollisionErrors() && $bCheckUpdated)
		{
			return $result;
		}
		else

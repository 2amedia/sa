...
final class CheckManager
{
		...
		if (!$cashboxList)
		{
			...
			if ($dbRes->fetch())
			{
				foreach ($entities as $entity)
				{
					...
					{
						$r = new Result();
						$r->addWarning(new Error(Loc::getMessage('SALE_CASHBOX_NOT_FOUND'), 'CASHBOX_NOT_FOUND'));
						EntityMarker::addMarker($order, $entity, $r);
	...
	public static function savePrintResult($checkId, array $data)
	{
		...
		if (isset($data['ERROR']))
		{
			...
			{
				CashboxCheckTable::update($checkId, array('STATUS' => 'E', 'DATE_PRINT_END' => new Main\Type\DateTime(), 'CNT_FAIL_PRINT' => 1));
...
//			'\Bitrix\Sale\Cashbox\SellOrderCheck',

...
class Logger
{
	 ...
	 * @throws \Exception
	 */
	public function addError($errCode, $itemId = NULL)
	{
...
//		add new error
		if (!$this->addErrorToTable($errCode, $itemId))
			return false;

//		notification for user about critical or execution errors
//		$vk = Vk::getInstance();
//		$profiles = $vk->getExportProfilesList(false);

//		show notify only for critical errors, other message - show only on page
		if ($errorDescription['CRITICAL'])
		{
//			early we generate CAdminNotify, but now they removed. See in repo
//			stop export process
	 ...
	 * @throws \Exception
	 */
	private function addErrorToTable($errCode, $itemId = NULL)
	{
		$fields = array(
...
//			add intro before errors
			if (strlen($errorsConvertedString) > 0)
			{
				...
				$result .= ':</b><br>' . $errorsConvertedString;
			}

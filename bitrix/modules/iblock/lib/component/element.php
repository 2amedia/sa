...
abstract class Element extends Base
{
	public function onPrepareComponentParams($params)
	{
		...
		$params['LINK_PROPERTY_SID'] = trim($params['LINK_PROPERTY_SID']);
		$params['LINK_ELEMENTS_URL'] = trim($params['LINK_ELEMENTS_URL']) ?: 'link.php?PARENT_ELEMENT_ID=#ELEMENT_ID#';
		...
		{
			$params['GIFTS_DETAIL_PAGE_ELEMENT_COUNT'] = 3;
		}

		if (empty($params['GIFTS_MAIN_PRODUCT_DETAIL_PAGE_ELEMENT_COUNT']))
		{
			$params['GIFTS_MAIN_PRODUCT_DETAIL_PAGE_ELEMENT_COUNT'] = 3;
		}
	...
	protected function editTemplateOfferProps(&$item)
	{
		...
		foreach ($item['OFFERS'] as $keyOffer => $offer)
		{
			...
			if (\CIBlockPriceTools::clearProperties($offer['DISPLAY_PROPERTIES'], $this->arParams['OFFERS_TREE_PROPS']))
			{

				...
				foreach($arResult['FIELDS'] as $num=>$f)
				{
					...
					if ($f['FIELD_TYPE']=='text') // simple input field
						echo '<div class="wizard_field_name">' . $f['NAME'] . ':</div>' . 
								'<input name="wizard['.$id.']" size=45 value="'.$f['FIELD_VALUE'].'">' . 
					...
					elseif ($f['FIELD_TYPE']=='multitext') // input options
					{
						...
						echo 	'<div class="wizard_field_name">' . $f['NAME'] . ':</div>' . 
								'<textarea name="wizard['.$id.']" rows=10 cols=45>'.$f['FIELD_VALUE'].'</textarea>' .
									'<br><font class=smalltext>' . $f['PREVIEW_TEXT'] . $link . '</font>';
	...
	<td>
		...
		<? if (count($arResult['SECTIONS'])) { ?>
			...
			<? } elseif ($arParams['BACK_URL']) {?>
				<input type=image src="<?=$templateFolder?>/images/button_back.gif" onclick="javascript:window.location='<?=$arParams['BACK_URL']?>';return false;">
			<? } ?>
		...
		<? } else { // Finish ?>
			<input type=image src="<?=$templateFolder?>/images/button_back.gif" name="back">
			<img src="/bitrix/images/1.gif" width=1 height=1>
			<input type=image src="<?=$templateFolder?>/images/button_done.gif" name="end_wizard">
		<? } ?>
		</p>
	</td>
	<td style="border-right:1px solid #BEC0CF"><img src="/bitrix/images/1.gif" width=1 height=1></td>

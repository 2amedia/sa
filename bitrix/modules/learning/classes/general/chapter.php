...
class CAllChapter
{
			...
			if (intval($arFields["CHAPTER_ID"])>0)
			{
				...
				if($ar = $r->Fetch())
				{
					...
					if ($ID)
					{
						...
						{
							$rsChildFolders = CChapter::GetTreeList($arthis["COURSE_ID"], $ID);
							$arChildrenIDs = array($ID);
							while($arFolder = $rsChildFolders->Fetch())
							{
								$arChildrenIDs[] = intval($arFolder["ID"]);
							}
							if ($ar["COURSE_ID"] != $arthis["COURSE_ID"])
								$arMsg[] = array("id"=>"CHAPTER_ID", "text"=> GetMessage("LEARNING_BAD_BLOCK_SECTION_ID_PARENT"));
							elseif (in_array($arFields["CHAPTER_ID"], $arChildrenIDs))
	...
	function Add($arFields)
	{
		...
		if ($this->CheckFields($arFields))
		{
			...
			if (
				...
				&& (
					!array_key_exists("MODULE_ID", $arFields["PREVIEW_PICTURE"])
					|| strlen($arFields["PREVIEW_PICTURE"]["MODULE_ID"]) <= 0
				)
			)
				$arFields["PREVIEW_PICTURE"]["MODULE_ID"] = "learning";
			...
			if (
				...
				&& (
					!array_key_exists("MODULE_ID", $arFields["DETAIL_PICTURE"])
					|| strlen($arFields["DETAIL_PICTURE"]["MODULE_ID"]) <= 0
				)
			)
				$arFields["DETAIL_PICTURE"]["MODULE_ID"] = "learning";
			...
			if ($ID && (is_set($arFields, "NAME") || is_set($arFields, "DETAIL_TEXT")))
			{
				if (CModule::IncludeModule("search"))
				{
					...
					{
						$arGroupPermissions = array_keys(CCourse::GetGroupPermissions($arChapter["COURSE_ID"]));
						...
						$arSearchIndex = Array(
							...
							"ENTITY_TYPE_ID" => "LEARN_CHAPTER",
							"ENTITY_ID" => $arPost["ID"],							
						);

						if ($arChapter["CHAPTER_ID"]) {
							$arSearchIndex["PARAM2"] = "H".$arChapter["CHAPTER_ID"];
							$arSearchIndex["ENTITY_TYPE_ID"] = "LEARN_CHAPTER";
							$arSearchIndex["ENTITY_ID"] = $arChapter["CHAPTER_ID"];
						}
	...
	function Update($ID, $arFields)
	{
		...
		if (is_set($arFields, "PREVIEW_PICTURE"))
		{
			if(strlen($arFields["PREVIEW_PICTURE"]["name"])<=0 && strlen($arFields["PREVIEW_PICTURE"]["del"])<=0 && strlen($arFields["PREVIEW_PICTURE"]["description"])<=0)
				unset($arFields["PREVIEW_PICTURE"]);
			else
			{
		...
		if (is_set($arFields, "DETAIL_PICTURE"))
		{
			if(strlen($arFields["DETAIL_PICTURE"]["name"])<=0 && strlen($arFields["DETAIL_PICTURE"]["del"])<=0 && strlen($arFields["DETAIL_PICTURE"]["description"])<=0)
				unset($arFields["DETAIL_PICTURE"]);
			else
			{

...
{
	return "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNMДЦЬдцьЯ";
}
...
function stemming_stop_de($sWord)
{
	...
	if(!$stop_list)
	{
		$stop_list = array (
			...
			"fьr"=>0, "gegen"=>0, "gewesen"=>0, "hab"=>0, "habe"=>0,
			...
			"kцnnen"=>0, "kцnnte"=>0, "machen"=>0, "man"=>0,
			...
			"soll"=>0, "sollte"=>0, "sondern"=>0, "sonst"=>0, "ьber"=>0, "um"=>0, "und"=>0,
			"uns"=>0, "unse"=>0, "unsem"=>0, "unsen"=>0, "unser"=>0, "unses"=>0,
			"unter"=>0, "viel"=>0, "vom"=>0, "von"=>0, "vor"=>0,
			"wдhrend"=>0, "war"=>0, "waren"=>0, "warst"=>0, "was"=>0,
			...
			"wollen"=>0, "wollte"=>0, "wьrde"=>0, "wьrden"=>0, "zu"=>0,
...
{
	$vowels = "AEIOUYДЦЬ";
	//First, replace Я by ss
	$word = str_replace("Я", "SS", $word); //Actually Я in uppercase is already SS
	...
	{
		$word = substr($word, 0, -strlen($match[1]));
	}
	//(b) e   en   es
	...
	elseif(substr($word_r1, -1) == "S" && preg_match("/[$s_ending]S$/", $word))
	{
		$word = substr($word, 0, -1);
	}
	...
	{
		$word = substr($word, 0, -strlen($match[2]));
	}

	//Finally,
	//turn U and Y back into lower case, and remove the umlaut accent from a, o and u.
	$word = str_replace(array("u", "y", "Д", "Ц", "Ь"), array("U", "Y", "A", "O", "U"), $word);
...
{
	return str_replace(array("Д", "Ц", "Ь"), array("A", "O", "U"), ToUpper($sText, "de"));
}
?>

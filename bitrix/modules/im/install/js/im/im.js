...
BX.IM = function(domNode, params)
{
	...
	this.revision = 93; // api revision - check include.php
...
BX.IM.prototype.startCallList = function(callListId, params)
{
	...
	if (!this.desktop.ready() && this.desktopStatus && this.desktopVersion >= 18)
	{
		...
		BX.desktopUtils.runningCheck(
			function()
			{
				BX.desktopUtils.goToBx("bx://calllist/id/"+callListId+stringParams)
			},
			BX.delegate(function()
			{
...
BX.IM.prototype.openMessenger = function(userId, tab)
{
	...
	this.popupMessengerContent = BX.create("div", { props : { className : "bx-messenger-box bx-messenger-mark bx-messenger-global-context-"+this.BXIM.context.toLowerCase()+" "+(this.webrtc.callInit? ' bx-messenger-call'+(this.callOverlayMinimize? '': ' bx-messenger-call-maxi'): '')+(BX.MessengerCommon.isPage()? ' bx-messenger-box-desktop': '')+(BX.browser.IsMac()? '': ' bx-messenger-custom-scroll') }, style: styleOfContent, children : [
		...
		this.popupMessengerDialog = BX.create("div", { props : { className : "bx-messenger-box-dialog" }, style : {marginLeft: this.popupContactListSize+'px'},  children : [
			...
			this.popupMessengerBodyDialog = BX.create("div", { props : { className : "bx-messenger-body-dialog bxu-file-input-over" }, children: [
				...
				this.popupMessengerBodyOlVote = BX.create("div", { props : { className : "bx-messenger-livechat-vote" }}),
					...
					this.popupMessengerFileButton = !this.disk.enable? null: BX.create("div", {attrs : { title: BX.message('IM_F_UPLOAD_MENU')}, props : { className : "bx-messenger-textarea-file"+(this.disk.lightVersion? " bx-messenger-textarea-file-light": "") }, children : [
						BX.create("div", { attrs: {'title': this.BXIM.ieVersion > 1? BX.message('IM_F_UPLOAD_MENU'): ' '}, props : { className : "bx-messenger-textarea-file-popup"+(this.BXIM.context == "LINES"? " bx-messenger-textarea-file-popup-short": "") }, children : [
							this.popupMessengerFileForm = BX.create('form', { attrs : { action : this.BXIM.pathToFileAjax, style: this.disk.lightVersion? "z-index: 0": ""}, props : { className : "bx-messenger-textarea-file-form" }, children: [
								...
								this.popupMessengerFileFormRegMessageHidden = BX.create('input', { attrs : { type : 'hidden', name: 'REG_MESSAGE_HIDDEN', value: 'N'}}),
								BX.create('input', { attrs : { type : 'hidden', name: 'IM_AJAX_CALL', value: 'Y'}}),
							...
							this.disk.lightVersion? null: BX.create("div", { props : { className : "bx-messenger-popup-menu-item" }, html: (BX.browser.IsMobile()? BX.message('IM_F_UPLOAD_MENU_1_M'): BX.message('IM_F_UPLOAD_MENU_1'))}),
							this.disk.lightVersion || this.BXIM.context == "LINES"? null: BX.create("div", { props : { className : "bx-messenger-menu-hr" }}),
...
BX.Messenger.prototype.tooltip = function(bind, text, params)
{
	...
	this.popupTooltip = new BX.PopupWindow('bx-messenger-tooltip', bind, {
		...
		zIndex: 2000,
		content : BX.create("div", { props : { style : "padding-right: 5px;"+(params.width>0? "width: "+params.width+"px;": '') }, html: text})
	});
	if (params.angle)
	{
...
BX.Messenger.prototype.openPopupMenu = function(bind, type, setAngle, params)
{
	...
	else if (type == 'openLinesMenu')
	{
		...
		offsetLeft = 14;
		menuItems = [
			{icon: 'bx-messenger-menu-pin', text: BX.message(session.pin == "Y"? "IM_M_OL_PIN_OFF": "IM_M_OL_PIN_ON"), onclick: BX.delegate(function(){  this.linesTogglePinMode();  this.closeMenuPopup(); }, this)},
			...
			{icon: 'bx-messenger-menu-close', text: BX.message("IM_M_OL_CLOSE"), onclick: BX.delegate(function(){  this.linesCloseDialog();  this.closeMenuPopup(); }, this)},
			{icon: 'bx-messenger-menu-history-2', text: BX.message("IM_M_HISTORY"), onclick: BX.delegate(function(){ this.openHistory(this.currentTab); this.closeMenuPopup(); }, this)}
		];
	}
	else if (type == 'status')
	...
	else if (type == 'dialogContext' || type == 'dialogMenu')
	{
		...
		var copyFile = this.message[messageId].params.FILE_ID && this.message[messageId].params.FILE_ID.length > 0 && BX.clipboard.isCopySupported();
		...
		menuItems = [
			...
			generalAccessBlock || selectedText.text.length > 0 || (!this.message[messageId].text && this.message[messageId].params.FILE_ID.length <= 0) ? null: {text: BX.message("IM_MENU_QUOTE2"), onclick: BX.delegate(function()
				{
					var arQuote = [];
			...
			selectedText.text.length > 0 || this.users[this.BXIM.userId].connector? null: {separator: true},
			selectedText.text.length > 0 || this.users[this.BXIM.userId].connector? null: {text: BX.message("IM_MENU_TO_TASK"), onclick: BX.delegate(function(){ this.shareMessage(messageId, 'TASK'); this.closeMenuPopup(); }, this)},
			selectedText.text.length > 0 || this.users[this.BXIM.userId].connector? null: {text: BX.message("IM_MENU_TO_POST"), onclick: BX.delegate(function(){ this.shareMessage(messageId, 'POST'); this.closeMenuPopup(); }, this)},
			...
			!canDelete? null: {text: BX.message("IM_M_HISTORY_DELETE"), onclick: BX.delegate(function() {this.deleteMessage(messageId);this.closeMenuPopup();}, this)}
		];
	}
...
BX.Messenger.prototype.shareMessage = function(messageId, type)
{
	BX.MessengerCommon.shareMessageAjax(messageId, type);
}
...
/* OPEN LINES */
BX.Messenger.prototype.linesLiveChatInit = function()
{
}
...
BX.Messenger.prototype.linesShowHistory = function(chatId, data)
{
	...
	this.popupHistoryPanel = null;
	var historyPanel = this.redrawHistoryPanel('chat'+chatId, chatId, {'drawLinesJoin': data.CAN_JOIN});
...
BX.Messenger.prototype.linesInitVote = function()
{
	this.popupMessengerBodyOlVote.innerHTML = '';
	BX.adjust(this.popupMessengerBodyOlVote, {children: [
		BX.create("div", { props : { className : "bx-messenger-livechat-vote-wrap" }, children: [
			BX.create("div", { props : { className : "bx-messenger-livechat-vote-text-offer" }, children: [
				BX.create("div", { props : { className : "bx-messenger-livechat-vote-text" }, html: BX.message('IM_OL_VOTE_TEXT')}),
				BX.create("div", { props : { className : "bx-messenger-livechat-vote-button" }, children: [
					BX.create("div", { attrs: {'title': BX.message('IM_OL_VOTE_LIKE')}, props : { className : "bx-messenger-livechat-vote-like" }}),
					BX.create("div", { attrs: {'title': BX.message('IM_OL_VOTE_DISLIKE')}, props : { className : "bx-messenger-livechat-vote-dislike" }})
				]})
			]}),
			BX.create("div", { props : { className : "bx-messenger-livechat-vote-text-done" }, children: [
				BX.create("div", { props : { className : "bx-messenger-livechat-vote-text" }, html: BX.message('IM_OL_VOTE_TNX')}),
			]})
		]})
	]});
}
		...
		if (BX.MessengerCommon.isDesktop())
		{
			...
			BX.desktop.addCustomEvent("BXProtocolUrl", BX.delegate(function(command, params) {
				...
				else if (command == 'calllist')
				{
					...
					if(!BX.type.isPlainObject(params.params))
						params.params = {};

					this.BXIM.startCallList(params.id, params.params);
				}
			}, this));
...
BX.IM.WebRTC.prototype.bindPhoneViewCallbacks = function(callView)
{
	...
	callView.setCallback('switchDevice', function()
	{

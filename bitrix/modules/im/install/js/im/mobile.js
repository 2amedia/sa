...
BX.ImMobile.prototype.mobileActionReady = function()
{
	...
	else if (this.mobileAction == 'DIALOG')
	{
		...
		BX.bindDelegate(this.messenger.popupMessengerBodyWrap, 'click', {className: 'bx-messenger-command'}, BX.delegate(function(e) {
			...
			{
				this.messenger.sendMessage(this.messenger.currentTab, BX.proxy_context.getAttribute('data-command'));
			}
			else if (BX.proxy_context.getAttribute('data-entity') == 'put')
			{
				BXMobileApp.UI.Page.TextPanel.setText(BX.proxy_context.getAttribute('data-command')+' ');
...
{
	if (!this.openChatFlag && this.users[this.currentTab].bot)
	{
		app.alert({'text': BX.message('IM_F_BOT_NOT_SUPPORT')});
		return false;
	}

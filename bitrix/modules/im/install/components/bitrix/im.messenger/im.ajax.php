...
else if ($_POST['IM_BOT_COMMAND'] == 'Y')
{
	...
	if($message = $orm->fetch())
	{
		...
		if (isset($relations[$userId]))
		{
			...
			{
				$chatId = intval(substr($_POST['DIALOG_ID'], 4));
			...
			else
			{
				$messageFields = Array(
					"FROM_USER_ID" => $userId,
					"TO_USER_ID" => intval($_POST['DIALOG_ID']),
					"MESSAGE"  => '/'.$_POST['COMMAND'].' '.$_POST['COMMAND_PARAMS'],
				);
			}
...
{
	$errorMessage = '';
	if(!CIMMessenger::Share($_POST['ID'], $_POST['TYPE']))
	{
...
else if ($_POST['IM_OPEN_LINES'] == 'Y' && CModule::IncludeModule('imopenlines'))
{

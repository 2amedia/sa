...
class CIMMessageParam
{
	...
	public static function PrepareValues($values)
	{
		...
		foreach($values as $key => $value)
		{
			...
			else if (in_array($key, Array('CHAT_ID', 'CHAT_MESSAGE', 'CHAT_LAST_DATE')))
			{
				$arValues[$key] = intval($value[0]);
			}
			else if ($key == 'CHAT_USER' || $key == 'FILE_ID' || $key == 'LIKE' || $key == 'KEYBOARD_ACTION' || $key == 'URL_ID' || $key == 'LINK_ACTIVE')
			{

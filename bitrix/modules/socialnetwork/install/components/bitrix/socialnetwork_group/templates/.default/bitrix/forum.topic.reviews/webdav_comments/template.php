...
?>
	<a href="#review_anchor" title="<?=GetMessage("F_QUOTE_HINT")?>" class="reviews-button-small" <?
			?>onMouseDown="toggleReviewForm(BX('forumCollapseToggler'), true); quoteMessageEx('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>', 'message_text_<?=$res["ID"]?>')"><?=GetMessage("F_QUOTE_FULL")?></a>
<?
		endif;
?>
		<a href="#review_anchor"  title="<?=GetMessage("F_NAME")?>"  class="reviews-button-small" <?
			?>onMouseDown="toggleReviewForm(BX('forumCollapseToggler'), true); reply2author('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>,')"><?=GetMessage("F_NAME")?></a></div>
<?
...
<?
$arUser = __parse_user($res["AUTHOR_ID"], $res["AUTHOR_URL"], (isset($arParams["NAME_TEMPLATE"])?$arParams["NAME_TEMPLATE"]:null));
$res["AUTHOR_NAME"] = isset($arUser["FULL_NAME"]) ? $arUser["FULL_NAME"] : $arUser["NAME"];
?>
<?
...
?>
<script>
    function toggleReviewForm(link, forceShow)
    {
        var form = BX.findChild(document, {'class':'reviews-reply-form'}, true);
        var display = form.style.display;
        if (display != 'block' || (forceShow != null))
        {
            form.style.display = 'block';
            link.innerHTML = '<?=CUtil::JSEscape(GetMessage("F_COLLAPSE_FORM"))?>';
        }
        else
        {
            form.style.display = 'none';
            link.innerHTML = '<?=CUtil::JSEscape(GetMessage("F_EXPAND_FORM"))?>';
        }
    }
</script>
<?
...
?>
<? if (sizeof($arResult["MESSAGES"]) > 0) { ?>
<div class="reviews-collapse"><a id="forumCollapseToggler" href="javascript:void(0);" onclick="toggleReviewForm(this);"><?=GetMessage("F_EXPAND_FORM")?></a></div>
<? } ?>
<div class="reviews-reply-form" style="display: <?=(sizeof($arResult["MESSAGES"])>0 ? 'none' : 'block')?> ">

...
arButtons['tab'] = ['BXButton',
	{
		...
		name : 'Р’СЃС‚Р°РІРёС‚СЊ Р·Р°РєР»Р°РґРєСѓ',
		handler : function ()
		{
			this.bNotFocus = true;
...
arButtons['bitrix_note'] = ['BXButton',
	{
		...
		name : 'Р’СЃС‚Р°РІРёС‚СЊ РїР°РјСЏС‚РєСѓ',
		handler : function()
		{
			this.bNotFocus = true;
...
'</table>';
	...
	return {
		title: "Р’СЃС‚Р°РІРёС‚СЊ РїР°РјСЏС‚РєСѓ",
		innerHTML : str,
		OnLoad: function()
		{
...
arEditorFastDialogs['tab'] = function(pObj)
{
	...
	'<tr>' +
		'<td align="right">' +
			'Р?РґРµРЅС‚РёС„РёРєР°С‚РѕСЂ Р·Р°РєР»Р°РґРєРё' + ':' +
		'</td>' +
		'<td>' +
	...
	'<tr>' +
		'<td align="right">' +
			'РќР°Р·РІР°РЅРёРµ Р·Р°РєР»Р°РґРєРё' + ':' +
		'</td>' +
		'<td>' +
...
'</table>';
	...
	var OnSave = function(t)
	{
		...
		if (name.length <= 0)
			return alert('РџРѕР»Рµ РЅР°Р·РІР°РЅРёРµ Р·Р°РєР»Р°РґРєРё РЅРµ РјРѕР¶РµС‚ Р±С‹С‚СЊ РїСѓСЃС‚С‹Рј');
		BXSelectRange(oPrevRange, pObj.pMainObj.pEditorDocument, pObj.pMainObj.pEditorWindow);
	...
	return {
		title: "Р’СЃС‚Р°РІРёС‚СЊ Р·Р°РєР»Р°РґРєСѓ",
		innerHTML : str,
		OnLoad: function()
		{
			window.oPrevRange = BXGetSelectionRange(pObj.pMainObj.pEditorDocument, pObj.pMainObj.pEditorWindow);
...
pPropertybarHandlers['bitrix_note'] = function (bNew, pTaskbar, pElement)
{
	...
	if(bNew)
	{
		...
		else
		{
			...
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'Р¦РµР»РµРІР°СЏ Р°СѓРґРёС‚РѕСЂРёСЏ:'}));
			...
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'Р?С… РІРѕРїСЂРѕСЃС‹:'}));

			cell = row.insertCell(-1);
			pTaskbar.arElements.questions = cell.appendChild(pTaskbar.pMainObj.CreateElement("TEXTAREA", {cols: '50'}));

			row = tProp.insertRow(-1); cell = row.insertCell(-1); cell.align = 'right'; cell.width="40%";
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'Р§РµРіРѕ С…РѕС‚РёРј РјС‹:'}));
			...
			row = tProp.insertRow(-1); cell = row.insertCell(-1); cell.align = 'right'; cell.width="40%";
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'РљРѕРјРјРµРЅС‚Р°СЂРёР№:'}));
...
pPropertybarHandlers['tab'] = function (bNew, pTaskbar, pElement)
{
	...
	if(bNew)
	{
		...
		else
		{
			...
			row = tProp.insertRow(-1); cell = row.insertCell(-1); cell.align = 'right'; cell.width="40%";
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'Р?РґРµРЅС‚РёС„РёРєР°С‚РѕСЂ Р·Р°РєР»Р°РґРєРё:'}));

			cell = row.insertCell(-1); cell.width="60%";
			pTaskbar.arElements.id = cell.appendChild(pTaskbar.pMainObj.CreateElement("INPUT", {size: '40'}));

			row = tProp.insertRow(-1); cell = row.insertCell(-1); cell.align = 'right';
			cell.appendChild(pTaskbar.pMainObj.CreateElement("SPAN", {innerHTML: 'РќР°Р·РІР°РЅРёРµ Р·Р°РєР»Р°РґРєРё:'}));
			cell = row.insertCell(-1);

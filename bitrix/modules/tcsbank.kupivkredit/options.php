...
if ($CAT_RIGHT>="R"):
	...
	if($CAT_RIGHT>="W" && check_bitrix_sessid())
	{
		...
		if($REQUEST_METHOD=="POST" && strlen($Update)>0)
		{
			...
			foreach($arOptions["OPTIONS"] as $sSiteID=>$arSite)
			{
				...
				if(!strlen($arRequestData["round"])) $arRequestData["round"]="round";
				...
				if(!isset($arRequestData["fast_form"])) $arRequestData["fast_form"]="n";
				COption::SetOptionString($sModuleID, "{$sSiteID}_open_widget", $arRequestData["open_widget"]);
				COption::SetOptionString($sModuleID, "{$sSiteID}_fast_form", $arRequestData["fast_form"]);
				...
				COption::SetOptionString($sModuleID, "{$sSiteID}", $sApiPort);
	...
	<?foreach($arOptions["OPTIONS"] as $sSiteID=>$arSite):?>
		<?$tabControl->BeginNextTab();?>
			...
			<tr>
				<td valign="top" width="50%">
					<?=GetMessage("TCS_FAST_FORM");?>
				</td>
				<td valign="top" width="50%">
					<input type = "checkbox" name = "<?=$sSiteID?>[fast_form]" value="y" <?=$arSite["fast_form"]=="y"?"checked":""?>/>
				</td>
			</tr>
			<tr>

...
if ($arRights > "D") 
{
	...
	if($iOrderID = IntVal($_REQUEST["ID"]))
	{
		if($obModule->CheckSiteRights(Array("ORDER_ID"=>$iOrderID)))
		{
			...
			{
				case "approve":
					$arReturn = $obModule->ConfirmOrder(
                        $iOrderID,
                        false,
                        htmlspecialchars($_REQUEST["COURIER_MODE"])
                    );
				...
				case "subscribe":
					...
					{
						$bOK = $obModule->OrderCompleted(
                            $iOrderID,
                            false,
                            htmlspecialchars($_REQUEST["CODE"]),
                            htmlspecialchars($_REQUEST["PERSON"])
                        );
					}
					else
					{
				...
				case "return":
					...
					$arReturn = $obModule->ReturnOrder(
                        $iOrderID,
                        htmlspecialchars($_REQUEST["RETURNED_AMOUNT"]),
                        htmlspecialchars($_REQUEST["CASH_RETURNED_TO_CUSTOMER"]),
                        false,
                        htmlspecialchars($_REQUEST["RETURN_TYPE"])
                    );

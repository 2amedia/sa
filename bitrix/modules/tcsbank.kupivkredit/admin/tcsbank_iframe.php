...
include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/sale/include.php");
include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/tcsbank.kupivkredit/include.php");
...
<div class = "dOrderFull">
	...
	else
	{
		...
		function NeedSave(bNeed)
		{
			if(bNeed)
			{
				TCSJquery('#sNeedSave').show();
				TCSJquery('#sOrderPrice').hide();
				TCSJquery(".sReform").attr("disabled","disabled");
			}
			else 
			{
				TCSJquery('#sNeedSave').hide();
				TCSJquery('#sOrderPrice').show();
				TCSJquery(".sReform").removeAttr("disabled");
			}
		}
		...
		function FillProductFields(index, arParams, iblockID)
		{
			...
			<div class = "dSlideWrapper">
				...
				<script>
					...
					var inpMonths = TCSJquery(".iMonths");
					var inpDownPayment = TCSJquery(".iDownPayment");
					var ssCreditSumm = TCSJquery(".sCreditSumm");
					var ssPerMonth = TCSJquery(".sPerMonth");					
					DivSlider1 = TCSJquery("#slider_down_payment");
					DivSlider1.slider({
						min:0,
						...
						{
							iValue = ui.value;
							iPercent = (iValue/TCSJquery(this).attr("max"))*100;
							TCSJquery(this).parents(".dSlideWrapper").find("input").val(iValue);
							TCSJquery(this).find(".ui-slider-range").css({"width":iPercent+"%"});
			...
			<div class = "dSlideWrapper">
				...
				<script>
					DivSlider2 = TCSJquery("#slider_month");
					...
					DivSlider2.slider({
						...
						{
							iValue = ui.value;
							iPercent = ((iValue-3)/(TCSJquery(this).attr("max")-TCSJquery(this).attr("min")))*100;
							TCSJquery(this).parents(".dSlideWrapper").find("input").val(iValue);
							TCSJquery(this).find(".ui-slider-range").css({"width":iPercent+"%"});
							iMonths = parseInt(inpMonths.val());
							iDownPayment = inpDownPayment.val();
		...
		<script language="JavaScript">
			...
			{
				TCSJquery('.iPay').removeAttr('disabled'); 
				MakeRequest(TCSJquery(obj).parents('form').serializeArray(),iOrderID);			

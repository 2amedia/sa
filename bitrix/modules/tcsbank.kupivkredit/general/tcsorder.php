<?
	class CTCSOrder extends CTCSOrderAll
	{
		...
		public function GetItemsArray($iOrderID)
		{
			$arReturn = Array(
				"ID"=>$iOrderID,
				"TOTAL_SUMM_RUB"=>0,
			...
			if($iOrderID = IntVal($iOrderID))
			{
				...
				if($arOrder = $obOrder->Fetch())
				{
					...
					$obItems = CSaleBasket::GetList(Array("ID"=>"ASC"),Array("ORDER_ID"=>$iOrderID),false,false,Array());
					$arItems = Array();
					$iTotalSumm = 0;
					$iTotalTCSSumm = 0;
					while($arItem = $obItems->Fetch())
					{
						...
						$arItem["DISCOUNT"] = CCurrencyRates::ConvertCurrency($arItem["DISCOUNT_PRICE"], $arItem["CURRENCY"],"RUB");

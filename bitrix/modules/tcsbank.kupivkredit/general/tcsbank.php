<?
	class CTCSBank extends CTCSExchange
	{
		...
		public function CTCSBank()
		{
			$this->sModuleID = "tcsbank.kupivkredit";
			//$this->sHost = "https://kupivkredit-test-api.tcsbank.ru:8100";
			$this->iKoef = 0.351/12;
			RegisterModuleDependences("sale", "OnSaleCancelOrder", $this->sModuleID, "CTCSBank", "OnCancelOrder");
			RegisterModuleDependences("sale", "OnBeforeOrderDelete", $this->sModuleID, "CTCSBank", "OnDeleteOrder");
			RegisterModuleDependences("main", "OnPageStart", $this->sModuleID, "CTCSBank", "ShowUserScripts");
			RegisterModuleDependences("sale", "OnSaleComponentOrderOneStepPaySystem", $this->sModuleID, "CTCSBank", "CheckPaySystem");
		}

		public function OnDeleteOrder($iOrderID)
		{
			$obModule = new CTCSBank;
			if($obModule->CheckOrderPaySystem($iOrderID)) $obModule->CancelOrder($iOrderID, false, "shop_decline");
		}

		public function OnCancelOrder($iOrderID, $sType)
		{
			if($sType=="Y")
			{
				$obModule = new CTCSBank;
		...
		public function GetHost($sHostCode, $sKey=false)
		{
		...
		public function CalculateMonthPayment($iMonths)
		{
			$iKoef = $this->iKoef;
		...
		public function MonthPayment($iMonths, $iCreditSumm)
		{
		...
		public function ShowUserScripts()
		{
            ...
            {
                $obTCSBank = new CTCSBank();
                global $APPLICATION;
                $sModuleID = $obTCSBank->sModuleID;
                $APPLICATION->AddHeadScript("/bitrix/js/{$sModuleID}/user_scripts.js");
            }
			if(isset($_REQUEST["CurrentStep"]))
			{
				CTCSBank::CheckPaySystem();
			}
		}

        public function CheckPaySystem($arResult)
        {
            if(!defined("ADMIN_SECTION") || ADMIN_SECTION!=true)
            {
                $obTCSBank = new CTCSBank();
                ...
                if(IntVal($_COOKIE["kupivkredit"]) && !isset($_POST["PAY_SYSTEM_ID"]))
                {
                    $arPaySystems = $obTCSBank->GetPaySystemsID();

                    if(!empty($arPaySystems) && ($iPaySystemID = IntVal($arPaySystems[0])))
                    {
						if($_REQUEST["CurrentStep"])
						{
							$_POST["PAY_SYSTEM_ID"]=$iPaySystemID;
						}
						else
						{
							foreach($arResult["PAY_SYSTEM"] as &$arPaySystem)
							{
								...
								if($iPaySystemID == $arPaySystem["ID"])
								{
		...
		public function PHPArrayToJS($arDest)
		{
			if (is_array($arDest))
			{
		...
		public function PluralForm($n, $forms) {
			return $n%10==1&&$n%100!=11?$forms[0]:($n%10>=2&&$n%10<=4&&($n%100<10||$n%100>=20)?$forms[1]:$forms[2]);
		}

		public function CheckOrderPaySystem($iOrderID)
		{
		...
		public function GetOrderTable($arFilter)
		{
			global $DB;
		...
		public function GetSiteSecret($sSiteID)
		{
			$arReturn = array(
		...
		public function GetPaySystemsID()
		{
		...
		public function PriceToPay($arParams)
		{
			IncludeModuleLangFile(__FILE__);
		...
		public function GetSitesData($sSiteID=false)
		{
			$arOptions = Array();
			$arFilter = ARray();
			...
			while($arSite = $obSites->Fetch())
			{
				$arParams = Array(
					...
					"fast_form"=>COption::GetOptionString($sModuleID, "{$arSite["LID"]}_fast_form","n"),
		...
		public function GetButton($iOrderID, $bNoButton=false)
		{
			global $APPLICATION;
			IncludeModuleLangFile(__FILE__);
			...
			$obOrder = CSaleOrder::GetList(Array(),Array("ID"=>$iOrderID),false,false,Array("ID","CURRENCY","PRICE","SUM_PAID","PERSON_TYPE_ID","PERSON_TYPE_ID","LID"));
			if($arOrder = $obOrder->Fetch())
			{
				...
				foreach($arData["ITEMS"] as $arItem)
				{
					if(IntVal($arItem["PRODUCT_ID"]))
					{
						...
						if($arElement = $obElement->Fetch())
						{
							...
							$iDiscount+=($arItem["DISCOUNT"]*$arItem["TCS_QUANTITY"]);
							$arProduct = Array(
								"title"=>$arItem["NAME"],
				...
				$iDiscount=$this->Round($iDiscount,$arOrder["LID"]);
				$details = Array();
				...
				$bFastOrder = $arSiteData["fast_form"]=="y"?'true':'false';
				$sResult .= "
					<script>
						vkredit = new VkreditWidget({$arSiteData["button"]}, {$iPriceToPay},  {
								order: '{$sBase64}',
								sig: '{$sSig}',
								isShortForm: {$bFastOrder}
						});
		...
		public function GetParam($iOrderID,$iSiteID, $iPersonType,$sField)
		{
			$sModuleID = $this->sModuleID;
		...
		public function RefreshOrder($iOrderID)
		{
			...
			$arResponseOrder = Array(
				...
				"LOAN_AMOUNT"=>$arReturn["result"]["LoanAmount"], // ����� ����� �������
				"MAX_LOAN_AMOUNT"=>$arReturn["result"]["MaxPossibleLoanAmount"], // ����������� ��������� ����� �������
				"MONTHLY_PAYMENT"=>$arReturn["result"]["MonthlyPayment"], // ����� ��������� �������
				"DOWN_PAYMENT"=>$arReturn["result"]["Downpayment"], // ����� ��������������� ������
				"SIGNING_DEADLINE"=>$DB->FormatDate($arReturn["result"]["ContractSigningDeadline"],"YYYY-MM-DD","DD.MM.YYYY"), // ����� ��������������� ������
				"DELIVERY_DEADLINE"=>$DB->FormatDate($arReturn["result"]["ContractDeliveryDeadline"],"YYYY-MM-DD","DD.MM.YYYY"), // ����� ��������������� ������
				"PAYMENT_COUNT"=>$arReturn["result"]["PaymentCount"], // ����� ��������
				"COMISSION"=>$arReturn["result"]["Commission"], // �������� �� ������ ������� (�������� � LoanAmount)
		...
		public function OnEndBufferContent(&$sContent)
		{
			if(!defined(ADMIN_SECTION))
			{
				$obTCSBank = new CTCSBank();
				if($obTCSBank->CheckSiteRights(Array("SITE_ID"=>SITE_ID)))
				{
					$iTime = microtime(1);
					$sButtonClass = "bx_cart";
					if(!substr_count($sContent,"TCSBuyClick"))
					{
						$sRegular = "#(?i)<a[^>]+{$sButtonClass}[^>]+>(.+?)</a>#";
						if(preg_match_all($sRegular,$sContent,$arMatches,PREG_OFFSET_CAPTURE))
						{
							$arLinks = $arMatches[0];
							...
							foreach($arLinks as $arLink)
							{
								...
								$sClass="";
								$sID="";
								if(preg_match("#class[^'\"]*['\"]([^'\"]*)['\"]#",$sLink,$arClass))
								{
									$sClass=$arClass[1];
								}
								if(preg_match("#id[^'\"0-9]*['\"]([^'\"]*)['\"]#",$sLink,$arID))
								{
									$sID=$arID[1];
								}
								IncludeModuleLangFile(__FILE__);
								$sAddLink = "<a href='javascript:void(0)' class='tcs-kvk-button' onclick='TCSBuyClick(\"{$sID}\")'></a>";
								$sContent = str_replace($sLink,$sLink.$sAddLink,$sContent);
							}
						}
					}
				}

...
BXBlockEditorDialogFileInput.prototype =
{
	getImages: function()
	{
		...
		var result = this.fileList.filter(function (file) {
			return BX.util.in_array(file.id, ids);
		}).map(function (file) {
			return file.url;
		});
	...
	onLink: function(data)
	{
		...
		this.onFileIsCreated = function(id, item)
		{
			...
			BX.addCustomEvent(item, 'onUploadDone', BX.delegate(function(item, data)
			{
				this.fileList.push({
					'id': data.file.id,
					'url': data.file.files.default.url + '&' + Math.random(),
		...
		for(var tabCode in this.caller.tabList)
		{
			...
			for(var fileTmp in data)
			{
				...
				var filtered = this.fileList.filter(function (file) {
					return fileTmp == file.path;
				});
				if (filtered.length > 0)
				{
					var file = filtered[0];
					item.ctrl.value = item.ctrl.value.replace(file.url, fileNew);
					file.saved = true;
				}
			}

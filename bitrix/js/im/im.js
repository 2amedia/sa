...
BX.IM = function(domNode, params)
{
	...
	this.revision = 21; // api revision - check include.php
	this.errorMessage = '';
...
{
	var whiteList = {'dialtone': true, 'ringtone': true};
...
BX.IM.prototype.stopRepeatSound = function(sound, clear)
{
	clear = clear !== false;
	if (clear && this.audio.timeout && this.audio.timeout[sound])
		clearTimeout(this.audio.timeout[sound]);
...
BX.IM.prototype.updateCounter = function(count, type)
{
	...
	if (this.desktop.ready())
	{
		...
		this.desktop.setIconBadge(sumLabel, this.messageCount > 0);
	}
...
BX.IM.prototype.openSettings = function(params)
{
	...
	this.settingsView.common = {
		...
		'settings': [
			...
			{'title': BX.message('IM_M_ENABLE_BIRTHDAY'), 'type': 'checkbox', 'checked': this.desktop.birthdayStatus(), 'callback': BX.delegate(function(){ this.desktop.birthdayStatus(!this.desktop.birthdayStatus()); }, this), 'condition': BX.delegate(function(){ return this.desktop.ready() }, this)},
			...
			{'type': 'space'},
			{'title': BX.message('IM_M_DESKTOP_AUTORUN_ON'), 'type': 'checkbox', 'checked': this.desktop.autorunStatus(), 'callback': BX.delegate(function(){ this.desktop.autorunStatus(!this.desktop.autorunStatus()); }, this), 'condition': BX.delegate(function(){ return this.desktop.ready() }, this)}
		]
	};
	...
	this.settingsView.privacy = {
		...
		'settings': [
			...
			{'title': BX.message('IM_SETTINGS_PRIVACY_SEARCH'), name: 'privacySearch', 'type': 'select', items: [{title: BX.message('IM_SETTINGS_SELECT_1_2'), value: 'all'}, {title: BX.message('IM_SETTINGS_SELECT_2_2'), value: 'contact'}], 'value': this.settings.privacySearch}
		]
	};
	...
	this.popupSettingsBody = BX.create("div", { props : { className : "bx-messenger-settings" }, children: this.prepareSettings(params.onlyPanel? {onlyPanel: params.onlyPanel}: {})});

	if (this.desktop.enableInVersion(15))
	{
		if (this.init)
		{
			this.desktop.openSettings(this.popupSettingsBody, "BXIM.openSettings();");
			return false;
		}
		else
		{
	...
	BX.bindDelegate(this.popupSettingsBody, 'click', {className: 'bx-messenger-settings-tab'}, BX.delegate(function() {
		...
		if (this.desktop.enableInVersion(15))
			this.desktop.autoResize('settings');
...
BX.IM.prototype.prepareSettings = function(params)
{
	...
	for (var tab in this.settingsView)
	{
		...
		tabs.push(BX.create('div', {attrs: {'data-id': i+""}, props : { className : "bx-messenger-settings-tab"+(firstTab? " bx-messenger-settings-tab-active": "") }, html: this.settingsView[tab].title, events: events}));
		firstTab = false;
		i++;
	}
	...
	for (var tab in this.settingsView)
	{
		...
		if (this.settingsView[tab].settings)
		{
			...
			{
				if (typeof(this.settingsView[tab].settings[item]) != 'object')
					continue;
		...
		tabs.push(BX.create("div", {style: {display: params.onlyPanel? (params.onlyPanel == tab? 'block': 'none'): '' }, props : { id: 'bx-messenger-settings-content-'+tab, className: "bx-messenger-settings-content"+(firstTab? " bx-messenger-settings-content-active": "")}, children: table}));
		firstTab = false;
	}
...
BX.IM.prototype.prepareSettingsTable = function(tab)
{
	...
	for (var item = 0; item < config.rows.length; item++)
	{
		...
		{
			if (typeof(config.rows[item][column]) != 'object')
				continue;
...
BX.IM.prototype.saveSettings = function(settings)
{
	...
	BX.localStorage.set('ims', true, this.settings);
...
BX.IM.prototype.saveFormSettings = function()
{
	...
	if (this.desktop.enableInVersion(15))
	{
		this.desktop.onCustomEvent("bxSaveSettings", [this.settings]);
	}
	else
	{
		BX.localStorage.set('ims', true, this.settings);
			...
			if (data.ERROR == "")
			{
				...
				for (var moduleId in data.VALUES)
				{
					...
					for (var notifyId in data.VALUES[moduleId])
					{
						...
						this.settingsNotifyBlocked[moduleId+"|"+notifyId] = false;
					}
				}
	...
	if (send && this.desktop.ready())
	{
		for (var i = 0; i < arNotify.length; i++)
		{
			...
			var messsageJs =
				...
				'BX.bind(BX.findChild(notify, {className : "bx-notifier-item-delete"}, true), "click", function(event){ if (this.getAttribute("data-notifyType") != 1) { BXIM.desktop.onCustomEvent("bxImClickCloseNotify", [this.getAttribute("data-notifyId")]); } BXIM.desktop.windowCommand("close"); BX.IM.preventDefault(event); });'+
				(arNotify[i].id>0? '': 'BX.bind(notify, "click", function(event){ BXIM.desktop.onCustomEvent("bxImClickNotify", []); BXIM.desktop.windowCommand("close"); BX.IM.preventDefault(event); });')+
...
BX.Notify.prototype.confirmRequest = function(params, popup)
{
	...
	BX.ajax({
		...
		onsuccess: BX.delegate(function() {
			...
			if (popup)
				this.desktop.windowCommand("close");
		}, this),
		onfailure: BX.delegate(function() {this.desktop.windowCommand("close");}, this)
	});
...
BX.Notify.prototype.blockNotifyType = function(settingName)
{
	...
	if (blockResult)
	{
		this.BXIM.settingsNotifyBlocked[settingName] = false;
	...
	if (this.BXIM.init)
	{
		...
		for(var userId in this.users)
		{
			if (this.users[userId].birthday && userId != this.BXIM.userId)
			{
				...
				{
					this.message[userId+'birthdayPopup'] = {'id' : userId+'birthdayPopup', 'senderId' : 0, 'recipientId' : userId, 'date' : BX.IM.getNowDate(true)-parseInt(BX.message("SERVER_TZ_OFFSET")), 'text' : BX.message('IM_M_BIRTHDAY_MESSAGE_SHORT').replace('#USER_NAME#', this.users[userId].name) };
					if (this.desktop.ready())
					{
...
BX.Messenger.prototype.openMessenger = function(userId)
{
	...
	else if (userId == 0)
	{
		...
		if (userId.toString().substr(0,4) == 'chat')
		{
			if (!(this.chat[userId.toString().substr(4)] && this.chat[userId.substr(4)].id))
				this.chat[userId.toString().substr(4)] = {'id': userId.toString().substr(4), 'name': BX.message('IM_M_LOAD_USER'), 'owner': 0, 'fake': true};
	...
	this.popupMessengerContent = BX.create("div", { props : { className : "bx-messenger-box"+(this.webrtc.callInit? ' bx-messenger-call': '') }, style: styleOfContent, children : [
		...
		this.popupContactListWrap = BX.create("div", { props : { className : "bx-messenger-box-contact" }, style : {width: this.popupContactListSize+'px'},  children : [
			...
			BX.create('div', {props : { className : "bx-messenger-cl-panel" }, children : [ BX.create('div', {props : { className : "bx-messenger-cl-panel-wrap" }, children : [
				...
				BX.create('span', {props : { className : "bx-messenger-cl-panel-right-wrap" }, children : [
					this.contactListPanelSettings = BX.create("span", { props : { title : BX.message("IM_SETTINGS"), className : "bx-messenger-cl-panel-settings-wrap"}})
				]})
			]}) ]})
		]}),
		/* DIALOG */
		this.popupMessengerDialog = BX.create("div", { props : { className : "bx-messenger-box-dialog" }, style : {marginLeft: this.popupContactListSize+'px'},  children : [
			this.popupMessengerPanel = BX.create("div", { props : { className : "bx-messenger-panel"+(this.openChatFlag? ' bx-messenger-hide': '') }, children : [
				...
				this.popupMessengerPanelCall1 = this.callButton(this.currentTab),
			...
			this.popupMessengerPanel2 = BX.create("div", { props : { className : "bx-messenger-panel"+(this.openChatFlag? '': ' bx-messenger-hide') }, children : [
				...
				this.popupMessengerPanelCall2 = this.callButton(this.currentTab),
	...
	if (!this.desktop.run() && this.BXIM.platformName != '' && this.BXIM.settings.bxdNotify)
	{
		clearTimeout(this.popupMessengerDesktopTimeout);
		this.popupMessengerDesktopTimeout = setTimeout(BX.delegate(function(){
			var acceptButton = BX.delegate(function(){
				window.open(BX.browser.IsMac()? "http://dl.bitrix24.com/b24/bitrix24_desktop.dmg?NOTIFY": "http://dl.bitrix24.com/b24/bitrix24_desktop.exe?NOTIFY", "desktopApp");
			...
			this.showTopLine(BX.message('IM_DESKTOP_INSTALL').replace('#WM_NAME#', this.BXIM.bitrixIntranet? BX.message('IM_BC'): BX.message('IM_WM')).replace('#OS#', this.BXIM.platformName), [{title: BX.message('IM_DESKTOP_INSTALL_Y'), callback: acceptButton},{title: BX.message('IM_DESKTOP_INSTALL_N'), callback: declineButton}],'install');
		}, this), 15000);
	}
	...
	BX.bind(this.contactListPanelSettings, "click", BX.delegate(this.BXIM.openSettings, this.BXIM));
...
BX.Messenger.prototype.callButton = function()
{
	return BX.create("span", {props : {className : this.callButtonStatus(this.currentTab)}, children: [
		BX.create("a", {
			...
			events : {
				click: BX.delegate(function(e){
					if (!this.webrtc.callInit)
						this.webrtc.callInvite(this.currentTab, true);
					BX.PreventDefault(e);
				}, this)
			}
		}),
	...
	for (var i in groups)
	{
		...
		if (userInGroup[i] && !userInGroup[i].isChat)
		{
			for (var j = 0; j < userInGroup[i].users.length; j++)
			{
				...
				if (user == undefined || this.BXIM.userId == user.id || typeof(user.name) == 'undefined' || exceptUsers[user.id])
					continue;
		...
		else if (userInGroup[i] && userInGroup[i].isChat)
		{
			for (var j = 0; j < userInGroup[i].users.length; j++)
			{
				...
				if (chat == undefined || chat.name == undefined)
					continue;
				...
				userItems.push(BX.create("a", {
					props : { className: "bx-messenger-cl-item bx-messenger-cl-status-online "+newMessage},
					attrs : { href:'#chat'+chat.id, 'data-userId' : 'chat'+chat.id, 'data-name' : chat.name, 'data-status' : 'online', 'data-avatar' : '' },
					html :  '<span class="bx-messenger-cl-count">'+newMessageCount+'</span>'+
							'<span class="bx-messenger-cl-avatar bx-messenger-cl-avatar-group"><img class="bx-messenger-cl-avatar-img" ><span class="bx-messenger-cl-status"></span></span>'+
...
BX.Messenger.prototype.openPopupMenu = function(bind, type)
{
	...
	else if (type == 'callMenu')
	{
		...
		menuItems = [
			...
			{icon: 'bx-messenger-menu-call-voice', text: BX.message('IM_M_CALL_VOICE'), onclick: BX.delegate(function(){ this.webrtc.callInvite(this.currentTab, false); this.closeMenuPopup(); }, this)},
	...
	else if (type == 'chatUser')
	{
		...
		{
			menuItems = [
				{icon: 'bx-messenger-menu-chat-put', text: BX.message('IM_M_CHAT_PUT'), onclick: BX.delegate(function(){ this.insertTextareaText(' '+BX.util.htmlspecialcharsback(this.users[userId].name)+', ', true); this.popupMessengerTextarea.focus(); this.closeMenuPopup(); }, this)},
				...
				(!this.webrtc.callSupport(userId, this) || this.webrtc.callInit)? null: {icon: 'bx-messenger-menu-video', text: BX.message('IM_M_CALL_VIDEO'), onclick: BX.delegate(function(){ this.webrtc.callInvite(userId, true); this.closeMenuPopup(); }, this)},
	...
	else if (type == 'contactList')
	{
		...
		if (this.recentList)
		{
			menuItems = [
				{icon: 'bx-messenger-menu-write', text: BX.message('IM_M_WRITE_MESSAGE'), onclick: BX.delegate(function(){ this.openMessenger(userId); this.closeMenuPopup(); }, this)},
				(userIsChat && (!this.webrtc.callSupport(userId, this) || this.webrtc.callInit))? null: {icon: 'bx-messenger-menu-video', text: BX.message('IM_M_CALL_VIDEO'), onclick: BX.delegate(function(){ this.webrtc.callInvite(userId, true); this.closeMenuPopup(); }, this)},
				{icon: 'bx-messenger-menu-history', text: BX.message('IM_M_OPEN_HISTORY'), onclick: BX.delegate(function(){ this.openHistory(userId); this.closeMenuPopup();}, this)},
				...
				userIsChat ? {icon: 'bx-messenger-menu-chat-rename', text: BX.message('IM_M_CHAT_RENAME'), onclick: BX.delegate(function(){ this.openMessenger(userId); this.renameChatDialog();  this.closeMenuPopup();}, this)}: {},
				userIsChat ? {icon: 'bx-messenger-menu-chat-exit', text: BX.message('IM_M_CHAT_EXIT'), onclick: BX.delegate(function(){ this.leaveFromChat(userId.toString().substr(4)); this.closeMenuPopup();}, this)}: {},
				userIsChat? {}: {icon: 'bx-messenger-menu-hide-'+(userIsChat? 'chat': 'dialog'), text: BX.message('IM_M_HIDE_'+(userIsChat? 'CHAT': 'DIALOG')), onclick: BX.delegate(function(){ this.recentListHide(userId); this.closeMenuPopup();}, this)}
			];
		}
		else
		{
...
BX.Messenger.prototype.deleteAllHistory = function(userId)
{
	...
	if (this.desktop.ready())
		this.desktop.onCustomEvent("bxImClearHistory", [userId]);
...
BX.Messenger.prototype.sendMessage = function(recipientId)
{
	...
	if (this.popupMessengerTextarea.value == '/clear')
	{
		...
		if (this.desktop.ready())
			console.log('NOTICE: User ('+this.BXIM.userId+') use /clear');

		return false;
	}
	else if (this.popupMessengerTextarea.value == '/webrtcDebug')
	{
		this.textareaHistory[this.currentTab] = '';
		this.popupMessengerTextarea.value = '';
		...
		if (this.desktop.ready())
			console.log('NOTICE: User ('+this.BXIM.userId+') use /webrtcDebug and TURN '+(this.webrtc.debug? 'ON': 'OFF')+' debug');
	...
	if (this.desktop.ready())
	{
		for (var i = 0; i < arNewMessage.length; i++)
		{
			...
			var messsageJs =
				...
				'BX.bind(notify, "click", function(){BXIM.desktop.onCustomEvent("bxImClickNewMessage", [notify.getAttribute("data-userId")]); BXIM.desktop.windowCommand("close")});'+
				'BX.bind(BX.findChild(notify, {className : "bx-notifier-item-delete"}, true), "click", function(event){ BXIM.desktop.onCustomEvent("bxImClickCloseMessage", [notify.getAttribute("data-userId")]); BXIM.desktop.windowCommand("close"); BX.IM.preventDefault(event); });'+
				'BX.bind(notify, "contextmenu", function(){ BXIM.desktop.windowCommand("close")});';
...
BX.Messenger.prototype.showTopLine = function(text, buttons, icon)
{
	...
	if (typeof (icon) == 'string')
		arElements.push(BX.create('span', { props : { className : "bx-messenger-box-topline-icon-"+icon }}));

	arElements.push(BX.create('span', { props : { className : "bx-messenger-box-topline-text" }, html: text}));
...
BX.Messenger.prototype.setKeyBind = function()
{
	if (this.bindFlag)
		return false;

	this.bindFlag = true;
	BX.bind(window, "keydown", this.bindAlt1 = BX.proxy(function(e) { if (e.keyCode == 49 && e.altKey == true){
		this.openMessenger(this.recentListIndex[0]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keyup", this.bindAlt2 = BX.proxy(function(e) { if (e.keyCode == 50 && e.altKey == true){
		this.openMessenger(this.recentListIndex[1]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt3 = BX.proxy(function(e) { if (e.keyCode == 51 && e.altKey == true){
		this.openMessenger(this.recentListIndex[2]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt4 = BX.proxy(function(e) { if (e.keyCode == 52 && e.altKey == true){
		this.openMessenger(this.recentListIndex[3]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt5 = BX.proxy(function(e) { if (e.keyCode == 53 && e.altKey == true){
		this.openMessenger(this.recentListIndex[4]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt6 = BX.proxy(function(e) { if (e.keyCode == 54 && e.altKey == true){
		this.openMessenger(this.recentListIndex[5]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt7 = BX.proxy(function(e) { if (e.keyCode == 55 && e.altKey == true){
		this.openMessenger(this.recentListIndex[6]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt8 = BX.proxy(function(e) { if (e.keyCode == 56 && e.altKey == true){
		this.openMessenger(this.recentListIndex[7]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt9 = BX.proxy(function(e) { if (e.keyCode == 57 && e.altKey == true){
		this.openMessenger(this.recentListIndex[8]);
		BX.PreventDefault(e);
	}}, this));
	BX.bind(window, "keydown", this.bindAlt0 = BX.proxy(function(e) { if (e.keyCode == 48 && e.altKey == true){
		this.openMessenger(this.recentListIndex[9]);
		BX.PreventDefault(e);
	}}, this));
};

BX.Messenger.prototype.removeKeyBind = function()
{
	this.bindFlag = false;
	BX.unbind(window, "keydown", this.bindAlt1); this.bindAlt1 = null;
	BX.unbind(window, "keydown", this.bindAlt2); this.bindAlt2 = null;
	BX.unbind(window, "keydown", this.bindAlt3); this.bindAlt3 = null;
	BX.unbind(window, "keydown", this.bindAlt4); this.bindAlt4 = null;
	BX.unbind(window, "keydown", this.bindAlt5); this.bindAlt5 = null;
	BX.unbind(window, "keydown", this.bindAlt6); this.bindAlt6 = null;
	BX.unbind(window, "keydown", this.bindAlt7); this.bindAlt7 = null;
	BX.unbind(window, "keydown", this.bindAlt8); this.bindAlt8 = null;
	BX.unbind(window, "keydown", this.bindAlt9); this.bindAlt9 = null;
	BX.unbind(window, "keydown", this.bindAlt0); this.bindAlt0 = null;
};
...
BX.IM.Desktop = function(BXIM, params)
{
	...
	else if (this.enable)
	{
		...
		BX.addCustomEvent("onPullError", BX.delegate(function(error) {
			if (error == 'AUTHORIZE_ERROR')
			{
				this.BXIM.openConfirm('<div class="im-desktop-reconnect">'+BX.message('IM_DESKTOP_RECONNECT')+'</div>', false);
				this.setIconStatus('offline');
				BXDesktopSystem.Login({});
			}
		}, this));
		BX.addCustomEvent("onImError", BX.delegate(function(error) {
			if (error == 'AUTHORIZE_ERROR')
			{
				this.BXIM.openConfirm('<div class="im-desktop-reconnect">'+BX.message('IM_DESKTOP_RECONNECT')+'</div>', false);
				this.setIconStatus('offline');
		...
		BX.addCustomEvent("onPullStatus", BX.delegate(function(status){
			if (status == 'offline')
				...
				this.setIconStatus(this.BXIM.settings.status);
		}, this));
		...
		BX.bind(window, "keydown", BX.delegate(function(e) {
			if (e.keyCode == 27)
			{
				if (this.messenger.popupChatDialog && this.messenger.popupChatDialogContactListSearch.value.length >= 0)
				{
				...
				{
					this.messenger.textareaHistory[this.currentTab] = '';
					if (BX.util.trim(this.messenger.popupMessengerTextarea.value).length <= 0 && !this.webrtc.callInit)
					{
						...
						this.windowCommand('hide');
					}
					else
						this.messenger.popupMessengerTextarea.value = "";
				}
			}
			else if (e.keyCode == 82 && (e.ctrlKey == true || e.metaKey == true))
			{
				location.reload();
				console.log('NOTICE: User ('+this.BXIM.userId+') use /windowReload');
			}
		...
		this.addCustomEvent("bxImClickNotify", BX.delegate(function() {
			this.BXIM.notify.openNotify();
		}, this));
		this.addCustomEvent("bxCallAnswer", BX.delegate(function(chatId, userId, video, callToGroup) {
			this.webrtc.callActive = true;
			BX.ajax({
				url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_ANSWER',
				method: 'POST',
				dataType: 'json',
				timeout: 10,
		...
		this.addCustomEvent("bxCallJoin", BX.delegate(function(chatId, userId, video, callToGroup) {
			this.callAbort();
			this.callOverlayClose(false);
			this.callInvite(callToGroup? 'chat'+chatId: userId, video);
			BXDesktopWindow.ExecuteCommand("show");
		}, this.webrtc));
		this.addCustomEvent("bxImClearHistory", BX.delegate(function(userId) {
		...
		this.addCustomEvent("bxSaveSettings", BX.delegate(function(settings) {
		...
		this.addCustomEvent("BXTrayConstructMenu", BX.delegate(function (){
			...
			BXDesktopWindow.AddTrayMenuItem({Id: "messenger",Highlight: true, Order: this.enableInVersion(15)? 10: 1,Title: (BX.message('IM_DESKTOP_OPEN_MESSENGER') || '').replace('#COUNTER#', (messengerCounter>0? '('+messengerCounter+')':'')), Callback: function(){
				BXIM.messenger.openMessenger(BXIM.messenger.currentTab);
				BXDesktopSystem.GetMainWindow().ExecuteCommand("show");
			},Default: true	});
			BXDesktopWindow.AddTrayMenuItem({Id: "notify",Order: this.enableInVersion(15)? 20: 2,Title: (BX.message('IM_DESKTOP_OPEN_NOTIFY') || '').replace('#COUNTER#', (notifyCounter>0? '('+notifyCounter+')':'')), Callback: function(){
				BXIM.notify.openNotify(false, true);
				BXDesktopSystem.GetMainWindow().ExecuteCommand("show");
			},Default: true	});
			BXDesktopWindow.AddTrayMenuItem({Id: "bdisk",Order: this.enableInVersion(15)? 30:3, Title: BX.message('IM_DESKTOP_BDISK'), Callback: function(){
				if (BXIM.desktop.diskAttachStatus())
					BXFileStorage.OpenFolder();
				else
					BXIM.desktop.changeTab(BX('desktop_tab_disk_header'));
				BXDesktopSystem.GetMainWindow().ExecuteCommand("show");
			}});
			BXDesktopWindow.AddTrayMenuItem({Id: "site",Order: this.enableInVersion(15)? 40:4, Title: (BX.message('IM_DESKTOP_GO_SITE') || '').replace('#COUNTER#', (lFcounter>0? '('+lFcounter+')':'')), Callback: function(){
				BXIM.desktop.openLF();
			}});
			BXDesktopWindow.AddTrayMenuItem({Id: "separator1",IsSeparator: true, Order: 50});
			BXDesktopWindow.AddTrayMenuItem({Id: "settings",Order: 60, Title: BX.message('IM_DESKTOP_SETTINGS'), Callback: function(){
				BXIM.openSettings();
				if (!BXIM.desktop.enableInVersion(15))
					BXDesktopSystem.GetMainWindow().ExecuteCommand("show");
			}});
			BXDesktopWindow.AddTrayMenuItem({Id: "separator2",IsSeparator: true,Order: 1000});
			BXDesktopWindow.AddTrayMenuItem({Id: "logout",Order: 1010, Title: BX.message('IM_DESKTOP_LOGOUT'),Callback: function(){ BXIM.desktop.logout() }});
			if (this.enableInVersion(15))
...
BX.IM.Desktop.prototype.supportSound = function()
{
	...
	return !this.enableInVersion(15) && this.enableInVersion(4);
};
...
BX.IM.Desktop.prototype.enableInVersion = function(version)
{
	...
	return this.getBuild() >= parseInt(version);
};
...
BX.IM.Desktop.prototype.windowCommand = function(command, currentWindow)
{
	...
	currentWindow = typeof(currentWindow) == 'undefined'? window: currentWindow;

	if (command == "hide" || command == "freeze" || command == "unfreeze")
	{
		currentWindow.BXDesktopWindow.ExecuteCommand(command);
	}
	else if (command == "close")
	{
		if (this.enableInVersion(17))
		{
			currentWindow.BXDesktopWindow.ExecuteCommand("close");
		}
		else
		{
			currentWindow.BXDesktopWindow.ExecuteCommand("hide");

			setTimeout(function(){
				currentWindow.BXIM.stopRepeatSound('ringtone');
				currentWindow.BXIM.stopRepeatSound('dialtone');
				currentWindow.close();
			}, 1000);
		}
	}
...
BX.IM.Desktop.prototype.adjustSize = function(change)
{
	...
	this.BXIM.messenger.popupContactListElementsSize = Math.max(this.BXIM.messenger.popupContactListElementsSize+newHeight, 279);
...
BX.IM.Desktop.prototype.autoResize = function(window)
{
	...
	var params = {};
	if (window == 'settings')
		params = { Width: this.minSettingsWidth, Height: document.body.offsetHeight };
	else
		params = { Width: this.BXIM.init? this.width: document.body.offsetWidth, Height: document.body.offsetHeight };

	BXDesktopWindow.SetProperty("clientSize", params);
};

BX.IM.Desktop.prototype.openSettings = function(content, js)
{
	if (!this.ready()) return false;

	BXDesktopSystem.GetWindow("settings", BX.delegate(function(settings) {
...
BX.IM.Desktop.prototype.diskAttachStatus = function(value)
{
	if (!this.ready()) return false;

	if (typeof(value) !='boolean')
	{
		if (this.diskAttach == null)
			this.diskAttach = this.BXIM.getLocalConfig("diskAttach", false);
	}
	else if (this.diskAttach == value)
	{
		this.diskAttach = value;
		this.BXIM.setLocalConfig("diskAttach", this.diskAttach);
	}
	return this.diskAttach;
};
...
BX.PopupWindowDesktop = function()
{
	...
	this.close = function(){ BXIM.desktop.windowCommand('close'); };
	this.destroy = function(){ BXIM.desktop.windowCommand('close'); };
};
	...
	if (this.enabled)
	{
		...
		BX.addCustomEvent("onPullEvent", BX.delegate(function(module_id,command,params) {
			if (module_id == "im")
			{
				if (command == 'call')
				{
					this.log('Incoming', params.command, JSON.stringify(params));
					...
					if (params.command == 'join')
					{
						...
						if (this.callInit || this.callActive)
						{
							setTimeout(BX.delegate(function(){
								BX.ajax({
									url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_BUSY',
									method: 'POST',
									dataType: 'json',
									timeout: 10,
						...
						else
						{
							...
							if (this.desktop.enableInVersion(15) && !this.BXIM.windowFocus)
							{
								...
								this.desktopCallWindow = BXDesktopSystem.ExecuteCommand('topmost.show.html', this.desktop.getHtmlPage("", "BXIM.webrtc.callNotifyWaitDesktop("+params.chatId+","+params.senderId+", "+(params.video?1:0)+", "+(params.callToGroup?1:0)+", true);", data, 'im-desktop-call'));
							}
						}
					}
					else if (params.command == 'invite' || params.command == 'invite_join')
					{
						...
						else
						{
							...
							if (this.desktop.enableInVersion(15) && !this.BXIM.windowFocus)
							{
								...
								this.desktopCallWindow = BXDesktopSystem.ExecuteCommand('topmost.show.html', this.desktop.getHtmlPage("", "BXIM.webrtc.callNotifyWaitDesktop("+params.chatId+","+params.senderId+", "+(params.video?1:0)+", "+(params.callToGroup?1:0)+");", data, 'im-desktop-call'));
							}
						}
					}
					...
					else if (this.callInit && this.callChatId == params.chatId && params.command == 'wait')
					{
					...
					else if (this.callActive && this.callChatId == params.chatId  && params.command == 'signaling')
					{
						...
						{
							if (!this.pc[params.senderId] && !this.pcStart[params.senderId])
								this.initPeerConnection(params.senderId);

							if (this.pc[params.senderId] == null)
								return false;

							this.pc[params.senderId].setRemoteDescription(new RTCSessionDescription(signal));
							this.pc[params.senderId].createAnswer(BX.delegate(function(desc){this.setLocalAndSend(params.senderId, desc)}, this), null, this.sdpConstraints);

						}
						else if (signal.type === 'answer' && this.pcStart[params.senderId])
						{
							if (this.pc[params.senderId] == null)
								return false;
						...
						else if (signal.type === 'candidate' && this.pcStart[params.senderId])
						{
							if (this.pc[params.senderId] == null)
								return false;

							var candidate = new RTCIceCandidate({sdpMLineIndex:signal.label, candidate:signal.candidate});
							this.pc[params.senderId].addIceCandidate(candidate);
						}
						else if (signal.type === 'bye' && this.pcStart[params.senderId])
						{
							this.callAbort();
							this.callOverlayClose();
						}
						...
						{
							this.log('Signaling command "'+params.command+'" from user "'+params.senderId+'" skip');
						}
					}
					...
					else if (this.callInit && this.callChatId == params.chatId  && params.command == 'busy_self')
					{
						this.callAbort();
						this.callOverlayClose();
					}
					...
					else if ((params.command == 'decline_self' && this.callChatId == params.chatId || params.command == 'answer_self' && !this.callActive) && !this.callDeclineSelfDisabled)
					{
					...
					else if (this.callInit && params.callToGroup && this.callChatId == params.chatId && (params.command == 'errorAccess' || params.command == 'waitTimeout' || params.command == 'busy' || params.command == 'decline'))
					{
						...
						if (this.callStreamUsers[params.senderId])
					...
					buttons : [
						{
							...
							events: {
								click : function()
								{
									window.open(BX.browser.IsMac()? "http://dl.bitrix24.com/b24/bitrix24_desktop.dmg?CALL": "http://dl.bitrix24.com/b24/bitrix24_desktop.exe?CALL", "desktopApp");
									this.callOverlayClose();
								}
							}
						},
...
{
	if (navigator.mozGetUserMedia && typeof(mozRTCPeerConnection) != 'undefined' && navigator.userAgent.substr(navigator.userAgent.indexOf('Firefox/')+8, 2) >= 26)
	{
		this.enabled = true;
...
BX.IM.WebRTC.prototype.toggleAudio = function(changeVariable)
{
	changeVariable = typeof(changeVariable) != 'undefined'? changeVariable: true;
	if (changeVariable)
		this.audioMuted = this.audioMuted? false: true;

	if (this.callStreamSelf && this.callStreamSelf.getAudioTracks().length>0)
	{
...
BX.IM.WebRTC.prototype.toggleVideo = function(changeVariable)
{
	changeVariable = typeof(changeVariable) != 'undefined'? changeVariable: true;
	if (changeVariable)
		this.videoMuted = this.videoMuted? false: true;

	if (this.callStreamSelf && this.callStreamSelf.getVideoTracks().length>0)
	{
...
BX.IM.WebRTC.prototype.startGetUserMedia = function(video, audio)
{
	...
	video = typeof(video) != 'undefined'? video: this.callToGroup? this.videoGroup: this.video;
	audio = typeof(audio) != 'undefined'? audio: true;

	var constraints = {"audio": audio, "video": this.callVideo ? {"mandatory": video, "optional": []}: false};
	this.log("Requested access to local media with constraints:\n  \"" + JSON.stringify(constraints) + "\"");
	try {
...
BX.IM.WebRTC.prototype.onUserMediaSuccess = function(stream)
{
	...
	if (this.callOverlayPhotoCompanion)
		this.callOverlayPhotoMini.src = this.callOverlayPhotoCompanion.src;
	...
	if (this.initiator)
	{
		if (this.callToGroup)
		{
			for (var i = 0; i < this.messenger.userInChat[this.callChatId].length; i++)
			{
				var userId = this.messenger.userInChat[this.callChatId][i];
				if (userId != this.BXIM.userId)
	...
	else
	{
		if (this.callToGroup)
		{
			...
			{
				var userId = this.messenger.userInChat[this.callChatId][i];
				if (userId != this.BXIM.userId && userId != this.callInitUserId && !this.callStreamUsers[userId])
				{
...
BX.IM.WebRTC.prototype.onUserMediaError = function(error)
{
	...
	this.log("Failed to get access to local media. Error code was " + error.code);
	if (this.callDialogAllow)
		this.callDialogAllow.close();
...
BX.IM.WebRTC.prototype.trace = function(text)
{
	...
	this.log((performance.now() / 1000).toFixed(3) + ": " + text);
};
...
BX.IM.WebRTC.prototype.callInvite = function(userId, video, screen)
{
	...
	if (!this.callSupport())
	{
		if (!this.desktop.ready())
		{
			this.BXIM.openConfirm(BX.message('IM_CALL_NO_WEBRT'), [
				BX.browser.IsMac()? null: new BX.PopupWindowButton({
					text : BX.message('IM_M_CALL_BTN_DOWNLOAD'),
					className : "popup-window-button-accept",
					events : { click : BX.delegate(function() { window.open(BX.browser.IsMac()? "http://dl.bitrix24.com/b24/bitrix24_desktop.dmg?CALL": "http://dl.bitrix24.com/b24/bitrix24_desktop.exe?CALL", "desktopApp"); BX.proxy_context.popupWindow.close(); }, this) }
				}),
				new BX.PopupWindowButton({
					text : BX.message('IM_NOTIFY_CONFIRM_CLOSE'),
					className : "popup-window-button-decline",
		...
		else if (!this.desktop.enableInVersion(15))
		{
			this.BXIM.openConfirm(BX.message('IM_CALL_DESKTOP_2'), [
				BX.browser.IsMac()? null: new BX.PopupWindowButton({
					text : BX.message('IM_M_CALL_BTN_UPDATE'),
					className : "popup-window-button-accept",
					events : { click : BX.delegate(function() { this.desktop.browse(BX.browser.IsMac()? "http://dl.bitrix24.com/b24/bitrix24_desktop.dmg?CALL": "http://dl.bitrix24.com/b24/bitrix24_desktop.exe?CALL", "desktopApp"); BX.proxy_context.popupWindow.close(); }, this) }
				}),
				new BX.PopupWindowButton({
					text : BX.message('IM_NOTIFY_CONFIRM_CLOSE'),
					className : "popup-window-button-decline",
	...
	if (!this.callActive && !this.callInit && userId > 0)
	{
		...
		BX.ajax({
			url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_INVITE',
			method: 'POST',
			dataType: 'json',
			timeout: 10,
			...
			onsuccess: BX.delegate(function(data)
			{
				if (data.ERROR == '')
				{
					...
					this.callInviteTimeout  = setTimeout(BX.delegate(function(){
						...
						this.callAbort(BX.message(data.CALL_TO_GROUP? 'IM_M_CALL_ST_NO_WEBRTC_1': 'IM_M_CALL_ST_NO_WEBRTC'));
						this.callOverlayButtons([{
							text: BX.message('IM_M_CALL_BTN_CLOSE'),
							className: 'bx-messenger-call-overlay-button-close',
							events: {
...
BX.IM.WebRTC.prototype.callWait = function()
{
	...
	this.callInviteTimeout = setTimeout(BX.delegate(function(){
		...
		this.callOverlayProgress('offline');
		this.callOverlayStatus(BX.message(this.callToGroup? 'IM_M_CALL_ST_NO_ANSWER_2': 'IM_M_CALL_ST_NO_ANSWER'));
		var callVideo = this.callVideo;
...
BX.IM.WebRTC.prototype.callInviteUserToChat = function(users)
{
	...
	BX.ajax({
		url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_INVITE_USER',
		method: 'POST',
		dataType: 'json',
		timeout: 60,
...
BX.IM.WebRTC.prototype.callCommand = function(chatId, command, params, async)
{
	...
	{
		BX.ajax({
			url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_SHARED',
			method: 'POST',
			dataType: 'json',
			timeout: 10,
			async: async,
				...
				events: {
					click : BX.delegate(function() {
						...
						else
						{
							...
							if (this.desktopCallWindow)
								this.desktop.windowCommand("close", this.desktopCallWindow);
						}
					}, this)
				}
			},
...
BX.IM.WebRTC.prototype.callNotifyWaitDesktop = function(chatId, userId, video, callToGroup, join)
{
		...
		buttons : [
			{
				...
				events: {
					click : BX.delegate(function() {
						if (join)
							this.desktop.onCustomEvent("bxCallJoin", [chatId, userId, video, callToGroup]);
						else
							this.desktop.onCustomEvent("bxCallAnswer", [chatId, userId, video, callToGroup]);
	...
	BXDesktopWindow.SetProperty('position',{X:STP_CENTER, Y:STP_VCENTER, Width: 470, Height: 120});
};

BX.IM.WebRTC.prototype.initPeerConnection = function(userId, initiator)
{
	...
	if (!this.pcStart[userId] && this.callStreamSelf /* && turnReady*/)
	{
		if (this.connected[userId])
		{
			...
			if (!this.callStreamSelf || !this.pc[userId])
				return false;

			this.log("Adding local stream.", JSON.stringify(this.pc[userId]), this.callStreamSelf);
...
BX.IM.WebRTC.prototype.doCall = function(userId)
{
	...
	this.pc[userId].createOffer(BX.delegate(function(desc){this.setLocalAndSend(userId, desc)}, this), null, constraints);
};
...
BX.IM.WebRTC.prototype.setLocalAndSend = function(userId, desc)
{
	...
	this.log('send offer', JSON.stringify(desc));

	BX.ajax({
		url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_SIGNALING',
		method: 'POST',
		dataType: 'json',
		timeout: 20,
...
BX.IM.WebRTC.prototype.createPeerConnection = function(userId)
{
	try {
		this.pc[userId] = new RTCPeerConnection(this.pcConfig, this.pcConstraints);
		this.pc[userId].onicecandidate = BX.delegate(function(event) { this.onIceCandidate(userId, event) }, this);
		this.pc[userId].onaddstream = BX.delegate(function(event) { this.onRemoteStreamAdded(userId, event) }, this);
    	this.pc[userId].onremovestream = BX.delegate(function(event) { this.onRemoteStreamRemoved(userId, event)}, this);

		this.log("Created RTCPeerConnnection with:\n" +
...
BX.IM.WebRTC.prototype.onRemoteStreamAdded = function (userId, event) {
	this.log('Remote stream added', JSON.stringify(event));
...
BX.IM.WebRTC.prototype.onIceCandidate = function (userId, event) {
	if (event.candidate)
	{
		BX.ajax({
			url: '/bitrix/components/bitrix/im.messenger/video.ajax.php?CALL_SIGNALING',
			...
			dataType: 'json',
			timeout: 10,
			data: {'IM_CALL' : 'Y', 'COMMAND': 'signaling', 'CHAT_ID': this.callChatId,  'RECIPIENT_ID' : userId, 'PEER': JSON.stringify({type: 'candidate', label: event.candidate.sdpMLineIndex, id: event.candidate.sdpMid, candidate: event.candidate.candidate}), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.bitrix_sessid()}
		});
	}
	else
	{
...
{
	var hrphoto = '';
	if (this.messenger.hrphoto[userId])
		hrphoto = this.messenger.hrphoto[userId];
	else if (this.messenger.users[userId].avatar == '/bitrix/js/im/images/blank.gif')
...
BX.IM.WebRTC.prototype.callDialog = function()
{
	...
	this.callOverlayButtons([
		...
		BX.browser.IsFirefox() || this.callScreen? null: {
		...
		!this.callVideo || this.desktop.run()? null: {
...
BX.IM.WebRTC.prototype.callOverlayShow = function(params)
{
	...
	this.callOverlay =  BX.create("div", { props : { className : 'bx-messenger-call-overlay '+(params.callToGroup? ' bx-messenger-call-overlay-group ':'')+(this.callOverlayMinimize? 'bx-messenger-call-overlay-mini': 'bx-messenger-call-overlay-maxi')}, style : callOverlayStyle, children: [
		...
		this.callOverlayBody = BX.create("div", { props : { className : 'bx-messenger-call-overlay-body'}, children: params.callToGroup? this.callGroupOverlayShow(params): this.callUserOverlayShow(params)})
	]});
	if (params.prepare)
	{
...
BX.IM.WebRTC.prototype.callGroupOverlayShow = function(params)
{
	...
	var callTitle = this.messenger.chat[callChatId].name;
	if (callTitle.length > 85)
		callTitle = callTitle.substr(0,85)+'...';
	var callTitle = BX.message('IM_CALL_GROUP_'+(params.video? 'VIDEO':'VOICE')+(callIncoming? '_FROM': '_TO')).replace('#CHAT#', callTitle);
	...
	return [
		...
		BX.create("div", { props : { className : 'bx-messenger-call-overlay-title'}, children: [
			BX.create("div", { props : { className : 'bx-messenger-call-overlay-title-block'}, html: callTitle})
		]}),
		...
		BX.create("div", { props : { className : 'bx-messenger-call-overlay-alfa'}, html: BX.message('IM_M_CALL_ST_ALFA')+' <span class="bx-messenger-call-overlay-webrtc"></span> WebRTC'})
	];
};
...
BX.IM.WebRTC.prototype.callUserOverlayShow = function(params)
{
	...
	var callTitle = callIncoming? BX.message('IM_M_CALL_'+(params.video? 'VIDEO':'VOICE')+'_FROM').replace('#USER#', this.messenger.users[params.fromUserId].name)
								: BX.message('IM_M_CALL_'+(params.video? 'VIDEO':'VOICE')+'_TO').replace('#USER#', this.messenger.users[params.toUserId].name);
...
BX.IM.WebRTC.prototype.callGroupOverlayRedraw = function()
{
	...
	if (this.currentTab != 'chat'+this.callChatId)
	{
		this.messenger.openMessenger('chat'+this.callChatId);
		BXIM.webrtc.callOverlayToggleSize(false)
	}
...
BX.IM.WebRTC.prototype.callOverlayClose = function(animation, onlyMarkup)
{
	...
	if (this.desktop.ready() && this.desktopCallWindow != null)
		this.desktop.windowCommand("close", this.desktopCallWindow);
};
...
BX.IM.WebRTC.prototype.callOverlayUpdatePhoto = function()
{
	...
	if (this.callOverlayPhotoCompanion)
	{
		if (this.messenger.hrphoto[this.callUserId])
			this.callOverlayPhotoCompanion.src  = this.messenger.hrphoto[this.callUserId];
		else if (this.messenger.users[this.callUserId].avatar == '/bitrix/js/im/images/blank.gif')
			this.callOverlayPhotoCompanion.src  = '/bitrix/js/im/images/hidef-avatar.png';
		else
			this.callOverlayPhotoCompanion.src  = this.messenger.users[this.callUserId].avatar;
	}
...
BX.IM.WebRTC.prototype.log = function()
{
	if (this.desktop.enableInVersion(16))
	{
		var text = '';
		...
		BXDesktopSystem.Log('video.'+this.BXIM.userEmail+'.log', text.substr(3));
	}
	else if (this.desktop.ready())
	{
		this.debug = true;
	...
	if (this.tryNativeNotify && window.webkitNotifications && window.webkitNotifications.checkPermission() == 1)
	{
		return BX.create("div", { props : { className : "bx-notifier-notify-confirm" }, children: [
			...
			BX.create("div", { props : { className : "popup-window-buttons" }, children: [
				...
				BX.create("div", { props : { className : "popup-window-button popup-window-button-link popup-window-button-link-cancel" }, events : { click : BX.delegate(function() { this.notify.closePopup(); this.BXIM.saveSettings({'tryInstallNotify': 'N'}); }, this)  }, children: [

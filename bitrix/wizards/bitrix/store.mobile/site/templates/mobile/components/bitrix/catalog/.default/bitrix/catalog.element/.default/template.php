			...
			<?if(is_array($arResult["OFFERS"]) && !empty($arResult["OFFERS"])):?>
					...
					<?foreach($arResult["OFFERS"] as $arOffer):?>
						...
						<?foreach($arParams["OFFERS_FIELD_CODE"] as $field_code):?>
							<div class="catalog-item-offers-field"><span><?echo GetMessage("IBLOCK_FIELD_".$field_code)?>:</span><br><?
									echo $arOffer[$field_code];?></div>
						<?endforeach;?>
						
						<?foreach($arOffer["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
							<div class="catalog-item-offers-field"><span><?=$arProperty["NAME"]?>:</span><br><?
								if(is_array($arProperty["DISPLAY_VALUE"]))
									echo implode(" / ", $arProperty["DISPLAY_VALUE"]);
								else
									echo $arProperty["DISPLAY_VALUE"];?></div>
						<?endforeach?>
						<?endif;?>
						<?foreach($arOffer["PRICES"] as $code=>$arPrice):?>
							<?if($arPrice["CAN_ACCESS"]):?>
								<div class="catalog-detail-price"><label><?=GetMessage("CATALOG_PRICE")?></label>&nbsp;&nbsp;
						...
						<?if($arOffer["CAN_BUY"]):?>
							<div class="catalog-detail-buttons">
								<a href="<?=$arOffer["BUY_URL"]?>" data-role="button" data-theme="c" data-inline="true"><?echo GetMessage("CATALOG_BUY")?></a>
								<a href="<?=$arOffer["ADD_URL"]?>" data-ajax="false" data-role="button" data-theme="c" data-inline="true" id="baskett"><?echo GetMessage("CATALOG_ADD_TO_BASKET")?></a>
							</div>

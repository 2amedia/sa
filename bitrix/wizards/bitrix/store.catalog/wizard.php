	...
	function ShowStep()
	{
		...
		if(is_array($PROPS))
		{
			foreach($PROPS as $key => $prop)
			{
				...
				if (isset($prop['DEL']))
					$this->content .= '<input type="hidden" name="PROPS['.$key.'][DEL]" value="'.$prop['DEL'].'">';
				if(is_array($prop['VALUES']) )
				{
					foreach($prop['VALUES'] as $key2 => $vv)
					{
						$this->content .= '<input type="hidden" name="PROPS['.$key.'][VALUES]['.$key2.']" value="'.$vv.'">';
					}  
				}
...
//		$this->SetPrevStep("step_props");
	...
	function ShowStep()
	{
		...
		if($PARAM["catalogId"] == 'new')
		{
			$rsIBlockTypes = CIBlockType::GetByID('catalog');
			if(!$arIBlockType = $rsIBlockTypes->Fetch())
			{
				...
				{
					$arFields['LANG'][$arLanguage['LID']]['NAME'] = GetMessage('WZD_IBLOCK_CATALOG');
				} 
			...
			$rsIBlockTypes = CIBlockType::GetByID('offers');
			if(!$arIBlockType = $rsIBlockTypes->Fetch())
			{
				global $DB;
				$arFields = Array(
				...
				while ($arLanguage = $rsLanguages->Fetch())
				{
					$arFields['LANG'][$arLanguage['LID']]['NAME'] = GetMessage('WZD_IBLOCK_SKU');
				}  
				$obBlocktype = new CIBlockType;
				$DB->StartTransaction();
				$res = $obBlocktype->Add($arFields);
		...
		if($IBLOCK_ID)
		{
			...
			if($PARAM["catalogId"] == 'new'){
				...
				// добавим типовые свойства RECOMMEND и MORE_PHOTO, MINIMUM_PRICE, MAXIMUM_PRICE
				...
				{
					//	добавим связь с торговыми предложениями
					$ID_SKU = CCatalog::LinkSKUIBlock($IBLOCK_ID, $IBLOCK_OFFERS_ID);
					CCatalog::Add(array('IBLOCK_ID' => $IBLOCK_OFFERS_ID, 'PRODUCT_IBLOCK_ID' => $IBLOCK_ID, 'SKU_PROPERTY_ID' => $ID_SKU));
				}
				else 
			...
			// добавим пользовательские свойства торговых предложений
			if($ONOFFERS == "Y" && $IBLOCK_OFFERS_ID)
			{
				$fOfferss = '';
				$arOffers = Array();
			...
			if($ONOFFERS == "Y")
			{
				// добавим настройки форм торговых предложений

...
<div class="section">
	<?
	function PrintPropsForm($arSource=Array(), $locationTemplate = ".default", $arProps)
	{
		if (!empty($arSource))
		{
			foreach($arSource as $id=>$arProperties)
			{
				if (in_array($id, $arProps))
				{
					?>
					<tr>
						<td class="name">
	...
	<table class="sale_order_table props" id="sale_order_props" <?=($bHideProps && $_POST["showProps"] != "Y")?"style='display:none;'":''?>>
		<?
		PrintPropsForm($arResult["ORDER_PROP"]["USER_PROPS_N"], $arParams["TEMPLATE_LOCATION"], array(3, 4, 5, 7, 8, 9));
		PrintPropsForm($arResult["ORDER_PROP"]["USER_PROPS_Y"], $arParams["TEMPLATE_LOCATION"], array(3, 4, 5, 7, 8, 9));
		?>
	</table>

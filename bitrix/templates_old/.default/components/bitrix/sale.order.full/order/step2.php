...
function PrintPropsForm($arSource=Array(), $PRINT_TITLE = "", $arParams)
{
	if (!empty($arSource))
	{
		...
		foreach($arSource as $arProperties)
		{
			...
			?>
			<? if($arProperties["NAME"] == "Местоположение") { ?>
			...
			<? } ?>
				...
				<td>
					...
					{
						?>
						<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?if($arProperties['NAME'] == "E-mail"):?><?=$_SESSION['SESS_AUTH']['EMAIL']?><?elseif($arProperties['NAME'] == "Контактное лицо"):?><?=$_SESSION['SESS_AUTH']['NAME']?><?else:?><?=$arProperties["VALUE"]?><?endif?>" name="<?=$arProperties["FIELD_NAME"]?>">
						<?
					}

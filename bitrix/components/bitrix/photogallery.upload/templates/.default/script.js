(function(window){
	...
	window.BXImageUploader.prototype = {
		Init: function()
		{
			...
			if (this.pForm && this.pAlbumSel)
			{
				...
				this.pNewAlbumName.onchange = this.pNewAlbumName.onblur = this.pNewAlbumName.onkeyup = function()
				{
					if (_this.pNewAlbumName && _this.pForm.new_album_name)
						_this.pForm.new_album_name.value = _this.pNewAlbumName.value;
				};
	...
	SimpleUploader.prototype = {
		...
		{
			var files = [];
			if ((!BX.browser.IsIE() || BX.browser.IsIE10()) && this.pInput.files && this.pInput.files.length > 0)
			{

...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	************************************/
	...
	)
	{
		...
		{
			$arOffersIblock = CIBlockPriceTools::GetOffersIBlock($iblockID);
			$OFFERS_IBLOCK_ID = is_array($arOffersIblock)? $arOffersIblock["OFFERS_IBLOCK_ID"]: 0;
			if(!$OFFERS_IBLOCK_ID)
				continue;
			$arElementsOffer = array();	
			foreach($arResult["ITEMS"] as $key=>$arElement)
				if ($arElement["IBLOCK_ID"] == $iblockID)
					$arElementsOffer[$key] = $arElement["ID"];
			...
			if(!empty($arOffers))
			{
				...
				foreach($arOffer as $key2=>$arOffer)
				{

		...
		function showLentaMenu(bindElement)
		{
			BX.PopupMenu.show(
				...
				[
					...
					if ($arParams["SHOW_SMART_FILTER_MYGROUPS"] == "Y")
					{
						?>,
						{ 
							text : "<?=GetMessageJS("SONET_C30_SMART_FILTER_MYGROUPS")?>", 
							className : "feed-sort-item<?=($arParams["MY_GROUPS_ONLY"] == "Y" ? " feed-sort-item-checked" : "")?>", 
							href : "<?=CUtil::JSEscape($GLOBALS["APPLICATION"]->GetCurPageParam("set_smart_filter_mygroups=".($arParams["MY_GROUPS_ONLY"] == "Y" ? "N" : "Y"), array("set_smart_filter_mygroups")))?>" 
						}
						<?
					}
					?>

...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		...
		<? 
			...
			$put_new = explode("/", $arItem["URL"]);
		?>
		<?if($put_new[2]=='derevostreet'){?>
			<a href="/<?=$put_new[1];?>/<?=$put_new[2];?>/<?=$put_new[4];?>/<?=$put_new[5];?>"><?echo $arItem["TITLE_FORMATED"]?></a>
		<?}else{?>
			<a href="<?=$arItem["URL"];?>"><?echo $arItem["TITLE_FORMATED"]?></a>
		<?}?>
		...
		<?endif;?>
		<?php /*?><small><?=GetMessage("SEARCH_MODIFIED")?> <?=$arItem["DATE_CHANGE"]?></small><br /><?php */?><?
